...
$table_name=getTableName($dept,$form);	
	...
	{	
		...
		if(!empty($results["communication_address"])){
			...
			$communication_address_str1=$communication_address->str1;$communication_address_str2=$communication_address->str2;$communication_address_vill=$communication_address->vill;$communication_address_dist=$communication_address->dist;$communication_address_pin=$communication_address->pin;$communication_address_m_no=$communication_address->m_no;$communication_address_email=$communication_address->email;
		}else{
		...
		<table class="table table-bordered table-responsive">			
			<tr>				
				<td>1. Full name of the Factory (with factory licence number if already registered before):</td>
							...
							<td>Name of the Factory :
							'.strtoupper($unit_name).'<br/>
						
							Legal Entity :
							'.strtoupper($legal_entity).'</td>
						 
					
				
			</tr>
			<tr>
				<td width="50%">2 (a) Full postal address and situation of the factory</td>
			...
			<tr>
				...
				if($owner_type=="PR"){ 
					$printContents=$printContents . strtoupper($owner_names);
				}
			   ...
			   $printContents=$printContents.'
				...
				<table>
					...
					<tr>
						...
						<td align="right">'.strtoupper($occupier_name).'<br/></td>				
					</tr>	
					<tr>
						<td>Date : </td>
					...
					<tr>
						...
						<td align="right">'.strtoupper($manager_name).'<br/></td>				
					</tr>	
					<tr>
						<td>Date : </td>
						<td align="right">'.date('d-m-Y',strtotime($results["sub_date"])).'</td>	
