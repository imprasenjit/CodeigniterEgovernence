...
function factory_fees_calculation($manuf_process_nat_fac,$manuf_prod_max_emp,$power_p){
	global $formFunctions,$factory;
	
	switch($manuf_process_nat_fac){
		case "PGS":
			if($manuf_prod_max_emp>0 && $manuf_prod_max_emp<=100){			
				$query=$factory->query("select fees_one from fees_schedule_b where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
				...
				$query=$factory->query("select fees_two from fees_schedule_b where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
				...
				$query=$factory->query("select fees_three from fees_schedule_b where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
				...
				$query=$factory->query("select fees_four from fees_schedule_b where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
		...
		case "ES":
			if($manuf_prod_max_emp>9){
				...
				$query=$factory->query("select fees_one from fees_schedule_c where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows > 0){
					$fees=$query->fetch_object()->fees_one;
				}else{
					$fees=0;
		...
		default: 
			if($manuf_prod_max_emp>0 && $manuf_prod_max_emp<21){			
				$query=$factory->query("select fees_one from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
					$fees=$query->fetch_object()->fees_one;
				}else{
					$fees=0;
				...
				$query=$factory->query("select fees_two from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
					$fees=$query->fetch_object()->fees_two;
				}else{
					$fees=0;
				...
				$query=$factory->query("select fees_three from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
					$fees=$query->fetch_object()->fees_three;
				}else{
					$fees=0;
				...
				$query=$factory->query("select fees_four from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
					$fees=$query->fetch_object()->fees_four;
				}else{
					$fees=0;
				...
				$query=$factory->query("select fees_five from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
				...
				$query=$factory->query("select fees_six from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
				...
				$query=$factory->query("select fees_seven from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
				...
				$query=$factory->query("select fees_eight from fees_schedule_a where capacity_min < '$power_p' and capacity_max > '$power_p'");
				if($query->num_rows>0){
