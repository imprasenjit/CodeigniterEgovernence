...
if(isset($_GET["token"]) && ($_GET["token"]=="2" || $_GET["token"]=="4")){
	$token=$_GET["token"];
	$check=$factory->query("select process_type from factory_form".$token." a,factory_form".$token."_process b,factory_form".$token."_certificates c where a.user_id='$swr_id' and b.form_id=a.form_id and b.process_type='A' and c.is_paid!='Y'") or die($factory->error);
	if($check->num_rows>0){
		$form_id=$factory->query("select form_id from factory_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->form_id;
		$fees=$factory->query("select total_fees from factory_form".$token."_certificates where form_id='$form_id' and is_paid!='Y'")->fetch_object()->total_fees;
		$prev="../index.php";
	}
			...
			<section class="content">
				...
				<div class="row">
					...
					<form name="fileUpload" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
						<input type="hidden" name="form_no" readonly="readonly" value="<?php echo $token;?>"/>
										...
										<div id="" style="display: block;">
											<div class="row">
												<div class="col-md-6 col-md-offset-4">
													...
													<p>Head of Account :<br/>"0230 Labour and Employment<br/>104 Fees Realised Under Factories Act"</p>
