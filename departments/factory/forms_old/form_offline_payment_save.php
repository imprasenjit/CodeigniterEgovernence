...
if(isset($_POST["submit_fees"])){
		...
		$tableName=$formFunctions->getTableName("factory",$form_no);
		$form_name=$formFunctions->get_formName("factory",$form_no);
		$form_id=$factory->query("select form_id from ".$tableName." where user_id='$swr_id' and active='1'")->fetch_object()->form_id;	
		if($_POST["payment_mode"]==1){
			...
			if(empty($_POST["offline_challan"]) || $_POST["offline_challan"]=='2' || $_POST["offline_challan"]=='3'){
				echo "<script>
					...
					$save_query=$factory->query("update ".$tableName."_certificates set offline_fees_challan='$offline_fees_challan',is_paid='Y' where form_id='$form_id'") or die($factory->error);
				if($save_query){
					
					$msgBody="Dear User, We have received your approval fees of ".$form_name.". Your License will be issued soon";
					...
					$dept_email="esgoa.factory@gmail.com";
					$sub="Licence fees reciept is received";
