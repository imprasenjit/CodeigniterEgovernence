...
*/
if($formName=="TRADE"){
	$sqltrade=$dma->query("select form_id,user_id from trade_license where form_no='$uian'");
	...
	if($sqltrade->num_rows>0){
		...
		#certificate check
		$is_paid="N";
		$sqltradeC=$dma->query("select is_paid from trade_license_certificates where form_id='$form_id'");
		$rowtradeC=$sqltradeC->fetch_array();
		if($sqltradeC->num_rows>0){
			$is_paid=$rowtradeC['is_paid'];
		}
		if($is_paid=="N"){
			$payment_insert=$dma->query("INSERT INTO trade_license_payment(form_id,CustomerID,TxnReferenceNo,BankReferenceNo,TxnAmount,BankID,BankMerchantID,TxnType,CurrencyName,TxnDate,AuthStatus,AdditionalInfo1,AdditionalInfo2,AdditionalInfo3,AdditionalInfo4,AdditionalInfo5,AdditionalInfo6,AdditionalInfo7,submitted_on) VALUES('$form_id','$CustomerID','$TxnReferenceNo','$BankReferenceNo','$TxnAmount','$BankID','$BankMerchantID','$TxnType','$CurrencyName','$TxnDate','$authStatus','$AdditionalInfo1','$AdditionalInfo2','$AdditionalInfo3','$AdditionalInfo4','$AdditionalInfo5','$AdditionalInfo6','$AdditionalInfo7','$submitted_on')") or die($mysqli->error);
			if($payment_insert){
				$save_query=$dma->query("update trade_license_certificates set is_paid='Y' where form_id='$form_id'") or die($dma->error);
