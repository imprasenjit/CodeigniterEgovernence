...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	if($q->num_rows > 0){
		...
		$form_id=$results["form_id"];$ulb=$results["ulb"];$from_year=$results["from_year"];$to_year=$results["to_year"];$family_name=$results["family_name"];$premises=$results["premises"];$godown=$results["godown"];$old_trade=$results["old_trade"];$annual_income=$results["annual_income"];$it_payable=$results["it_payable"];$license_type=$results["license_type"];			
		...
		if($old_trade_details_b!="" && $old_trade_details_b!="0000-00-00"){
			$old_trade_details_b = date('d-m-Y',strtotime($old_trade_details_b));
		}else{
			$old_trade_details_b="";
		}
		if($old_trade_details_c!="" && $old_trade_details_c!="0000-00-00"){
			$old_trade_details_c = date('d-m-Y',strtotime($old_trade_details_c));
		}else{
			$old_trade_details_c="";
		}
		...
		if($results["license_type"]=="G"){
			$license_type="REGULAR (1 YEAR)";
		}elseif($results["license_type"]=="P"){
	...
	if(!isset($css)){
		...
		<table class="table table-bordered table-responsive">
			...
			<tr><td colspan="2" height="45px" align="center"><b>I. DETAILS OF THE TRADE/OWNER </b></td></tr>
				<tr>
					<td valign="top" width="35%">(a) Applicant&apos;s Name </td>
					<td width="50%">'.strtoupper($key_person).'</td>
			
				</tr>
				<tr>
					<td valign="top" width="35%">     (b)    Applicant&apos;s PAN CARD Number </td>
					<td width="50%">'.strtoupper($pan_no).'</td>
			
				</tr>
				
				<tr>
					<td valign="top" > Name of trade / shop  </td>
					<td>				'.strtoupper($trade_name).'  </td>
				</tr>
				
				<tr>
					<td valign="top" > Owner&rsquo;s Type  </td>
					<td>				'.strtoupper($owner_type).' </td>
				</tr>
				
				<tr>
					<td valign="top" > Owner&rsquo;s Name  </td>
					<td>				'.strtoupper($owner_names).' </td>
				</tr>
				<tr>
					<td valign="top"> Father&rsquo;s / Spouse&rsquo;s name </td>
					<td>				'.strtoupper($results["family_name"]).' </td>
				</tr>
				<tr>
					<td valign="top"> ULB </td>
					<td>'.strtoupper($ulb).'</td>
				</tr>
				<tr>
					<td colspan="2">
					<table class="table table-bordered table-responsive">
						<tr>
							<td width="25%"> (a) Owner&rsquo;s Date of Birth :</td><td width="25%">'.strtoupper($results["dob"]).'</td>
							<td width="25%">(b) Owner&rsquo;s Age :</td><td width="25%">'.strtoupper($results["owner_age"]).'</td>
						</tr>
					</table>
					</td>
				</tr>
				<tr>
					<td valign="top">  Address of the Trade  </td>
					<td>
						<table class="table table-bordered table-responsive">
							<tbody>
								<tr>
				...
				<tr>
					<td valign="top">  Address of the Owner  </td>
					<td >
						<table class="table table-bordered table-responsive">
							<tbody>
								<tr>
				...
				<tr>
					<td colspan="2">
						<table class="table table-bordered table-responsive">
							<tr>	
								<td width="25%"> (a) Phone No. </td><td width="25%">'.strtoupper($b_landline_std).'-'.strtoupper($b_landline_no).'</td>
								<td width="25%">(b) Mobile No. </td><td width="25%">+91 '.strtoupper($mobile_no).'</td>
							</tr>
					</table>
					</td>
				</tr>
				<tr><td colspan="2" height="45px" align="center"><b>II. TYPES OF PREMISES </b></td></tr>
				<tr>
					<td valign="top" >   Type of premises ?</td>
				...
				
				{
					$printContents=$printContents.'
				 <tr>
					<td valign="top" width="35%"> dma Holding No. </td>
				...
				<tr>
					<td valign="top" > (a) Rent Per Month  </td>
					<td>				'.strtoupper($premises_details_c).' </td>
				</tr>
				<tr>
					<td valign="top" > (b) Does premises have parking space ?      </td>
					<td>				'.strtoupper($premises_details_d).' </td>
				</tr>
				
				<tr>
					<td valign="top" > Owner&rsquo;s Address  </td>
				...
				<tr>
					<td valign="top" > Owner&rsquo;s Residence Address  </td>
					<td>				'.strtoupper($premises_details_e).' </td>
				</tr>';
				}
				...
				<tr>
					<td valign="top" >   Does Godown Exist in the same premises ? </td>
				...
				
				{
					$printContents=$printContents.'
				<tr>
					<td valign="top" > Godown Address  </td>
					<td>				SAME PREMISES</td>
				</tr>
				
				<tr>
					<td valign="top" > (a) Rent Per Annum  </td>
					<td>				'.strtoupper($godown_details_b).'</td>
				</tr>
				<tr>
					<td valign="top" >   (b) Does godown have parking space ?      </td>
				...
				$printContents=$printContents.'
				<tr>
					<td colspan="2" height="45px" align="center"><b>IV. OLD TRADE LICENSE DETAILS </b></td></tr>
				<tr>
					<td valign="top" >  Do you have existing Trade License ? </td>
				...
				<tr>
					<td valign="top" > License Number  </td>
					<td>'.strtoupper($old_trade_details_a).' </td>
				</tr>
				
				<tr>
					<td valign="top" > Date of Issue (Most recent) </td>
					<td>'.strtoupper($old_trade_details_b).' </td>
				</tr>
				<tr>
					<td valign="top" > 1st License Issue Date  </td>
				...
				<tr>
					<td valign="top" width="50%"> Type of Business  </td>
					<td width="50%">'.strtoupper($business_type).' </td>
				</tr>
				
				<tr>
					<td valign="top" > Capital Investment  </td>
					<td>'.strtoupper($cap_investment).' </td>
				</tr>
				<tr>
					<td valign="top" > Annual Income for the previous<br/>financial year (in Rupees)    </td>
					<td>				'.strtoupper($results["annual_income"]).' </td>
				</tr>
				<tr>
					<td valign="top" > Income Tax Payable / Paid for <br/>the previous financial year (in Rupees) : </td>
					<td>	'.strtoupper($results["it_payable"]).' </td>
				</tr>
				<tr>
					<td valign="top" > License:(Regular /Provisional (1 Year)) </td>
