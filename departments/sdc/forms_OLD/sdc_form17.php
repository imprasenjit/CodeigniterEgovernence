...
$check=$formFunctions->is_already_registered('sdc','17');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=17&dept=sdc';		</script>";
}else if($check==3){
	echo "<script>window.location.href = 'payment_section.php?token=17';</script>";
}else{
	$showtab="";
}
...
include "save_form.php";
	...
	$owners=explode(",",$Name_of_owner); 
		$q=$sdc->query("select * from sdc_form17 where user_id='$swr_id' and active='1'");
		$results=$q->fetch_array();
		if($q->num_rows<1){	
			$form_id="";$cosmetics_names="";$co_name="";
			$file1="";$file2="";$file3="";$file4="";
			
			
		}else{			
			$form_id=$results["form_id"];$cosmetics_names=$results["cosmetics_names"];$co_name=$results["co_name"];
	}
...
<head>
	...
	<?php include ("sdc_form17_Addmore.php"); ?> <!-- File handles 'Addmore' Operation -->
</head>
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from sdc_form_names where form_no='17'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
									...
									<tr>
										...
										<tr>
											...
											<table name="objectTable1" id="objectTable1" class="table table-responsive text-center" >
												...
												<?php
													$part1=$sdc->query("SELECT * FROM sdc_form17_t1 WHERE form_id='$form_id'");
													$num1 = $part1->num_rows;
													if($num1>0){
													  $count=1;
										...
										<tr>
											<td colspan="2">1. A true copy from the letter from the manufacturing concern whose manufacturing capacity is intend to be utilized by the applicant.</td>
											<td>
										...
										<tr>
											<td colspan="2">2. A true copy if the letter from the manufacturing concern that they agree to lend the service of their expert staff equipments and premises for the manufacture of each attempt required by the applicant and they will analyse every batch of and maintain the register of raw materials finish products and report of analysis separately on behalf.</td>
											<td>
										...
										<tr>
											...
											<td class="text-center" colspan="2">
												<button type="submit" style="font-weight:bold" name="save17" class="btn btn-success">Save and Next</button>
											</td>
											<td></td>
										</tr>
									</table>
