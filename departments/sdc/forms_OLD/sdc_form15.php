...
$check=$formFunctions->is_already_registered('sdc','15');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=15&dept=sdc';		</script>";
}else if($check==3){
	echo "<script>window.location.href = 'payment_section.php?token=15';</script>";
}else{
	$showtab="";
}
...
include "save_form.php";
	...
	$owners=explode(",",$Name_of_owner); 
		$q=$sdc->query("select * from sdc_form15 where user_id='$swr_id' and active='1'");
...
<head>
	...
	<?php include ("sdc_form15_Addmore.php"); ?> <!-- File handles 'Addmore' Operation -->
</head>
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from sdc_form_names where form_no='15'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
									...
									<table id="" class="table table-responsive">
										...
										<tr>
											...
											<td colspan="3"><table name="objectTable1" id="objectTable1" class="table table-responsive text-center" >
												...
												<?php
													$part1=$sdc->query("SELECT * FROM sdc_form15_t1 WHERE form_id='$form_id'");
													$num = $part1->num_rows;
													if($num>0){
													  $count=1;
										...
										<tr>
											...
											<table name="objectTable2" id="objectTable2" class="table table-responsive text-center" >
												...
												<?php
													$part2=$sdc->query("SELECT * FROM sdc_form15_t2 WHERE form_id='$form_id'");
													$num2 = $part2->num_rows;
													if($num2>0){
													  $count=1;
										...
										<tr>
											<td colspan="2">1. Location, Building, Water supply should be comply with schedule M.</td>
											<td>
										...
										<tr>
											<td colspan="2">2. Approved plan of premises as per Schedule F Part XII- C.</td>
											<td>
										...
										<tr>
											<td colspan="2">6. Documents relating to ownership or tenancy of the premises. (Join inspections with CDSCO officials and Staff officials).</td>
											<td>
										...
										<tr>
											...
											<td class="text-center" colspan="2">
												<button type="submit" style="font-weight:bold" name="save15" class="btn btn-success">Save and Next</button>
											</td>
											<td></td>
										</tr>
									</table>
