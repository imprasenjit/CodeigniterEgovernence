...
$check=$formFunctions->is_already_registered('sdc','10');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=10&dept=sdc';
		</script>";
}else if($check==3){
	echo "<script>window.location.href = 'payment_section.php?token=10';</script>";
}else{
	$showtab="";
}
...
include "save_form.php";
	...
	$owners=explode(",",$Name_of_owner); 
		$q=$sdc->query("select * from sdc_form10 where user_id='$swr_id' and active='1'");
		...
		if($q->num_rows<1){	
			...
			$form_id=$results["form_id"];$drug=$results["drug"];
...
<head>
	...
	<?php include ("sdc_form10_Addmore.php"); ?> <!-- File handles 'Addmore' Operation -->
</head>
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from sdc_form_names where form_no='10'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
									...
									<table id="" class="table table-responsive">
										...
										<tr>
											...
											<table name="objectTable1" id="objectTable1" class="table table-responsive text-center" >
												...
												<?php
													$part1=$sdc->query("SELECT * FROM sdc_form10_t1 WHERE form_id='$form_id'");
													$num = $part1->num_rows;
													if($num>0){
													  $count=1;
										...
										<tr>
											<td colspan="2">8. Proof of compliance of sch. M- of D7C Act, 1940 & Rules.</td>
											<td>
										...
										<tr>
											<td class="text-center" colspan="4">
												<button type="submit" style="font-weight:bold" name="save10" class="btn btn-success">Save and Next</button>
											</td>
										</tr>
									</table>
								</form>
