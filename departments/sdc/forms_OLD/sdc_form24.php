...
$check=$formFunctions->is_already_registered('sdc','24');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=24&dept=sdc';
		</script>";
}else if($check==3){
	echo "<script>window.location.href = 'payment_section.php?token=24';</script>";
}else{
	$showtab="";
}
...
include "save_form.php";
	...
	$owners=explode(",",$Name_of_owner); 
		$q=$sdc->query("select * from sdc_form24 where user_id='$swr_id' and active='1'");
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from sdc_form_names where form_no='24'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
									...
									<table id="" class="table table-responsive">
										...
										<tr>
											...
											<td class="text-center" colspan="2">
												<button type="submit" style="font-weight:bold" name="save24" class="btn btn-success">Save and Next</button>
											</td>
											<td></td>
										</tr>
									</table>
