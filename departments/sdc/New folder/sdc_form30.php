...
$check=$formFunctions->is_already_registered('sdc','30');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=30&dept=sdc';
		</script>";
}else if($check==3){
	echo "<script>window.location.href = 'payment_section.php?token=30';</script>";
}else{
	$showtab="";
}
...
include "save_form.php";
	...
	$owners=explode(",",$Name_of_owner); 
		$q=$sdc->query("select * from sdc_form30 where user_id='$swr_id' and active='1'");
...
<head>
	...
	<?php include ("sdc_form30_Addmore.php"); ?> <!-- File handles 'Addmore' Operation -->
</head>
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from sdc_form_names where form_no='30'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
									...
									<table id="" class="table table-responsive">
										...
										<tr>
											<td class="text-center" colspan="4">
												<button type="submit" style="font-weight:bold" name="save30" class="btn btn-success">Save and Next</button>
											</td>
										</tr>
									</table>
								</form>
