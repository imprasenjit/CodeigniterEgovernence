...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	$form_id=$_GET["ui"];
	$q=$sdc->query("select * from sdc_form40  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else if(isset($_GET["uain"])){
	$uain=$_GET["uain"];
	$q=$sdc->query("select * from sdc_form40  where user_id='$swr_id' and uain='$uain'") or die("Error :".$sdc->error);	
}else if(isset($form_id)){
	$form_id=$form_id;
	$q=$sdc->query("select * from sdc_form40  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else{
	$q=$sdc->query("select * from sdc_form40  where user_id='$swr_id' and active='1' and form_id=form_id") or die("Error :".$sdc->error);
}
	...
	$form_name=$formFunctions->get_formName('sdc','40');
		if(!isset($css)){
		...
		if(!empty($results["uain"])){
				$printContents=$printContents.'<p style="text-align:right;padding:40px 40px 0 0;"> UAIN: '.strtoupper($results["uain"]).'</p>';
		}
		...
		<tr>
			<td colspan="2">
				<table width="100%" align="center" class="table table-bordered table-responsive" style="border-collapse: collapse" border="1">
					...
					$results1=$sdc->query("select * from sdc_form40_members where form_id='$form_id'") or die("Error : ".$sdc->error);
					$sl=1;
		...
		<tr>
			<td>40. Name of the trade or professional Association of which the applicant is a member and the date of commencement of memberships : </td>
