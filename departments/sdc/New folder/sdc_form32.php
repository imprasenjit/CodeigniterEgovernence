...
$check=$formFunctions->is_already_registered('sdc','32');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=32&dept=sdc';
		</script>";
}else if($check==3){
	echo "<script>window.location.href = 'payment_section.php?token=32';</script>";
}else{
	$showtab="";
}
...
include "save_form.php";
	...
	$owners=explode(",",$Name_of_owner); 
		$q=$sdc->query("select * from sdc_form32 where user_id='$swr_id' and active='1'");
...
<head>
	...
	<?php include ("sdc_form32_Addmore.php"); ?> <!-- File handles 'Addmore' Operation -->
</head>
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from sdc_form_names where form_no='32'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
									...
									<table id="" class="table table-responsive">
										...
										<tr>
											...
											<table name="objectTable1" id="objectTable1" class="table table-responsive text-center" >
												...
												<?php
													$part1=$sdc->query("SELECT * FROM sdc_form32_t1 WHERE form_id='$form_id'");
													$num = $part1->num_rows;
													if($num>0){
													  $count=1;
										...
										<tr>
											...
											<td class="text-center" colspan="2">
												<button type="submit" style="font-weight:bold" name="save32" class="btn btn-success">Save and Next</button>
											</td>
											<td></td>
										</tr>
									</table>
