...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	$form_id=$_GET["ui"];
	$q=$sdc->query("select * from sdc_form26  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else if(isset($_GET["uain"])){
	$uain=$_GET["uain"];
	$q=$sdc->query("select * from sdc_form26  where user_id='$swr_id' and uain='$uain'") or die("Error :".$sdc->error);	
}else if(isset($form_id)){
	$form_id=$form_id;
	$q=$sdc->query("select * from sdc_form26  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else{
	$q=$sdc->query("select * from sdc_form26  where user_id='$swr_id' and active='1' and form_id=form_id") or die("Error :".$sdc->error);
}
	...
	$form_name=$formFunctions->get_formName('sdc','26');
		if(!isset($css)){
		...
		<h4 align="center">'.$assamSarkarLogo.'<br/>FORM-VI<br/>[RULE 69]<br/>'.$form_name.'</h4>
		<br>
		<table width="99%" align="center" class="table table-bordered table-responsive" style="margin:0px auto;">
		<tbody>	
		<tr>
			<td colspan="2">1.I/We  &nbsp;'.strtoupper($key_person).'&nbsp; of &nbsp;'.strtoupper($unit_name).'&nbsp; hereby apply for the grant/renewal of a licence to manufacture on the premises situated at&nbsp;'.strtoupper($b_dist).'&nbsp; the following drugs of the than those specified in [Schedules C, C(1) and X] to the Drugs and Cosmetics Rules, 1945.</td>
		</tr>
		<tr>
			<td width="50%">2. Names of drugs categorized according to Schedule M :</td>
			<td width="50%">'.strtoupper($drug_name).'</td>
		</tr>
		<tr>
			<td >3. Names, qualifications and experience of technical staff employed for manufacture and testing.</td>
			...
			$part1=$sdc->query("SELECT * FROM sdc_form26_t1 WHERE form_id='$form_id'");
