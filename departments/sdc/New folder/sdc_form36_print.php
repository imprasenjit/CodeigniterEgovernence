...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	$form_id=$_GET["ui"];
	$q=$sdc->query("select * from sdc_form36  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else if(isset($_GET["uain"])){
	$uain=$_GET["uain"];
	$q=$sdc->query("select * from sdc_form36  where user_id='$swr_id' and uain='$uain'") or die("Error :".$sdc->error);	
}else if(isset($form_id)){
	$form_id=$form_id;
	$q=$sdc->query("select * from sdc_form36  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else{
	$q=$sdc->query("select * from sdc_form36  where user_id='$swr_id' and active='1' and form_id=form_id") or die("Error :".$sdc->error);
}
	...
	$form_name=$formFunctions->get_formName('sdc','36');
		if(!isset($css)){
		...
		<h4 align="center">'.$assamSarkarLogo.'<br/>FORM 31<br/>[Rule 139]<br/>'.$form_name.'</h4>
		<br>
		<table width="99%" align="center" class="table table-bordered table-responsive" style="margin:0px auto;">
		<tbody>	
		<tr>
			<td colspan="2">1.I/We  &nbsp;'.strtoupper($key_person).'&nbsp; of M/s&nbsp;'.strtoupper($unit_name).'&nbsp; hereby apply for the grant renewal of a licence to manufacture on the premises situated at&nbsp;'.strtoupper($b_dist).'</td>
		</tr>
		<tr>
		...
		<tr>
			...
			$part1=$sdc->query("SELECT * FROM sdc_form36_t1 WHERE form_id='$form_id'");
