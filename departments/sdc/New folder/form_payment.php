...
$save_mode="D";
if($token=="1"){
	$check=$formFunctions->is_already_registered('sdc','1');
	if($check==1){
		echo "<script>
	...
	$form_name=$cms->query("select form_name from sdc_form_names where form_no='1'")->fetch_object()->form_name;
	$sql=$sdc->query("select form_id,uain,save_mode from sdc_form8 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$check=$formFunctions->is_already_registered('sdc','2');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
	...
	$sql=$sdc->query("select form_id,uain,save_mode from sdc_form2 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$sql=$sdc->query("select form_id,uain,save_mode from sdc_form3 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$sql=$sdc->query("select form_id,uain,save_mode from sdc_form4 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
...
$returnUrl=$server_url."departments/sdc/forms/form_payment.php?token=".$token;
...
if($sid==18){
$newData="DOIACGOA|".$sentCustomerID."|NA|1.00|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|sdc001|".$txDateTime."|".$form_name."|NA|NA|".$returnUrl;
}else{
$newData="DOIACGOA|".$sentCustomerID."|NA|100.00|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|sdc001|".$txDateTime."|".$form_name."|NA|NA|".$returnUrl;
}
...
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=1&dept=sdc" />
<?php }
	if($formName=="F2"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=2&dept=sdc" />
<?php } 
	if($formName=="F3"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=3&dept=sdc" />
<?php } 
	if($formName=="F4"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=4&dept=sdc" />
<?php } ?>  
<?php } ?> 
</head>
