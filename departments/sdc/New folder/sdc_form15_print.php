...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	$form_id=$_GET["ui"];
	$q=$sdc->query("select * from sdc_form15  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else if(isset($_GET["uain"])){
	$uain=$_GET["uain"];
	$q=$sdc->query("select * from sdc_form15  where user_id='$swr_id' and uain='$uain'") or die("Error :".$sdc->error);	
}else if(isset($form_id)){
	$form_id=$form_id;
	$q=$sdc->query("select * from sdc_form15  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$sdc->error);
}else{
	$q=$sdc->query("select * from sdc_form15  where user_id='$swr_id' and active='1' and form_id=form_id") or die("Error :".$sdc->error);
}
	...
	$form_name=$formFunctions->get_formName('sdc','15');
		if(!isset($css)){
		...
		<tbody>	
		<tr>
			<td colspan="2">1.I/We  &nbsp;'.strtoupper($key_person).'&nbsp; of M/s&nbsp;'.strtoupper($unit_name).'&nbsp;  hereby apply for the grant of licence/renewal of licence number &nbsp;&nbsp;'.strtoupper($license_no).'&nbsp;&nbsp; dated :&nbsp;&nbsp;'.strtoupper($license_date).'&nbsp;&nbsp; to manufacture blood products on the premises situated at&nbsp;&nbsp;'.strtoupper($b_dist).'</td>
		</tr>
		<tr>
		...
		<tr>
			<td >3.   The name(s), qualification and experience of competent technical staff as under  :</td>
			<td >	<table width="100%" align="center" class="table table-bordered table-responsive" style="margin:0px auto;border-collapse: collapse" border="1">
			<tr align="center">
				<td align="center">Sl No </td>
				<td align="center"> Name </td>
		...
		<tr>
			<td colspan="2">4. The premises and plant are ready  for inspection/will be ready for inspection on '.strtoupper($inspection).' </td>
		</tr>
		';			
