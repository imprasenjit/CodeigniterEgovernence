...
if($q->num_rows > 0){
	...
	$form_id=$results["form_id"];$auth_person=$results["auth_person"];$drug_name=$results["drug_name"];$prev_lic_no=$results["prev_lic_no"];
	if($results["inspection_date"]!="" || $results["inspection_date"]!='00-00-0000' || $results["inspection_date"]!='0000-00-00'){
		$inspection_date=date('d-m-Y',strtotime($results["inspection_date"]));
	}else{
		$inspection_date="";
	}
...
$form_name=$formFunctions->get_formName($dept,$form);
	...
	<table class="table table-bordered table-responsive">
		...
		<tr>
			<td width="25%">Licence No.</td>
			<td width="25%">'.strtoupper($prev_lic_no).'</td>
		</tr>
		<tr>
			<td>Issue date</td>
