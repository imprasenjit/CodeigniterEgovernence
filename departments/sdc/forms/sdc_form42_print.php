...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	if($q->num_rows > 0){
		...
		$form_id=$results["form_id"];$auth_person=$results["auth_person"];$drug_name=$results["drug_name"];$staff_manuf=$results["staff_manuf"];$prev_lic_no=$results["prev_lic_no"];
	...
	<table class="table table-bordered table-responsive">	
		<tr>
			<td colspan="2">1. I/We  &nbsp;'.strtoupper($auth_person).'&nbsp; of &nbsp;'.strtoupper($unit_name).'&nbsp;hereby apply for the grant/renewal of a licence to manufacture on the premises situated at&nbsp;&nbsp;'.strtoupper($b_dist).'&nbsp;&nbsp; the undermentioned drugs, specified in Schedules C, C(1) and X to the Drugs and Cosmetics Rules, 1945.</td>
		</tr>
		<tr>
			<td width="50%">2. Names of drugs each substance to be separately specified :</td>
			<td width="50%">'.strtoupper($drug_name).'</td>
		</tr>
		<tr>
			<td >3. The names, qualifications and experience of the expert actually connected with the manufacture and testing of specified products in the manufacturing premises.</td>
