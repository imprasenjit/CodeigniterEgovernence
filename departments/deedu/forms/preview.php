...
if(isset($_GET["token"]) && is_numeric($_GET["token"]) && $_GET["token"]>0 && $_GET["token"]<40 ){
	$token=$_GET["token"];	
	$check=$formFunctions->is_already_registered('deedu',$token);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=deedu';
			</script>";	
	}else if($check==0){
		echo "<script>
	...
	include "deedu_form".$token."_print.php";
}else{
	echo "<script>
			alert('Something went wrong!!!');
			window.location.href = '".$server_url."departments/deedu/';
	</script>";
} ?>
<!DOCTYPE html>
<html>
  ...
  <div class="container-fluid avoid_me">
	...
	<div class="navbar-header col-md-12 text-center">	
	  ...
	  <?php if(isset($show_edit)){ ?>
			<a type="button" href="deedu_form<?php echo $token; ?>.php" class="btn btn-app bg-light-blue"><i class="fa fa-edit"></i>Edit</a>
