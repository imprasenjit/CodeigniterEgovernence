...
if(isset($_POST["save2a"])){		
	...
	$row=$sql->fetch_array();			
	if($sql->num_rows<1){   ////////////table is empty//////////////				
		$query=$formFunctions->executeQueryInsertID($dept,"insert into ".$table_name."(user_id,sub_date,inst_location,affil,instruct,is_comm_act,is_inst_estd,land_status,is_scheme,recog,edu_level,education_stage,land) values ('$swr_id','$today', '$inst_location', '$affil','$instruct', '$is_comm_act', '$is_inst_estd', '$land_status', '$is_scheme', '$recog', '$edu_level', '$education_stage', '$land')");
	}else{
		$form_id=$row["form_id"];	
		$query=$formFunctions->executeQuery($dept,"update ".$table_name." set sub_date='$today', inst_location='$inst_location', affil='$affil',instruct='$instruct', is_comm_act='$is_comm_act', is_inst_estd='$is_inst_estd', land_status='$land_status', is_scheme='$is_scheme', recog='$recog', edu_level='$edu_level',education_stage='$education_stage', land='$land' where form_id=$form_id");		
	}				
	if($query){
		$formFunctions->insert_incomplete_forms($dept,$form); 
		echo "<script>
...
if(isset($_POST["save3a"])){		
	...
	$row=$sql->fetch_array();			
	if($sql->num_rows<1){   ////////////table is empty//////////////				
		$query=$formFunctions->executeQueryInsertID($dept,"insert into ".$table_name."(user_id,sub_date,authority,name_of_indiv,location,date_of_prior,date_of_reg,stage_of_edu,education_stage,steam_n_subjects,medium_of_ins,recognized_school) values ('$swr_id','$today', '$authority', '$name_of_indiv','$location', '$date_of_prior', '$date_of_reg','$stage_of_edu','$education_stage','$steam_n_subjects', '$medium_of_ins', '$recognized_school')");
	}else{
		$form_id=$row["form_id"];	
		$query=$formFunctions->executeQuery($dept,"update ".$table_name." set sub_date='$today', authority='$authority', name_of_indiv='$name_of_indiv',location='$location', date_of_prior='$date_of_prior', date_of_reg='$date_of_reg',stage_of_edu='$stage_of_edu',education_stage='$education_stage',steam_n_subjects='$steam_n_subjects', medium_of_ins='$medium_of_ins',recognized_school='$recognized_school' where form_id=$form_id");		
	}				
	if($query){
		$formFunctions->insert_incomplete_forms($dept,$form); 
		echo "<script>
