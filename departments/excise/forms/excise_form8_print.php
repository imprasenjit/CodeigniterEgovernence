...
$table_name=getTableName($dept,$form);
	...
	if($q->num_rows>0){	
		...
		$is_citizen=$results["is_citizen"];$is_criminal=$results["is_criminal"];$proposed_plant=$results["proposed_plant"];$apparatus_description=$results["apparatus_description"];$plant_site=$results["plant_site"];$ten_date=$results["ten_date"];$esti_quantity=$results["esti_quantity"];$is_servant=$results["is_servant"];$is_pollution=$results["is_pollution"];
			...
			if(!empty($results["capacity_p"])){
				$capacity_p=json_decode($results["capacity_p"]);
				if(isset($capacity_p->a)) $capacity_p_a=$capacity_p->a; 
		...
		if($capacity_p_a=="A")
		{
		  $capacity_p_a="Upto 80 Lakh BL";
		}
		elseif($capacity_p_a=="B")
		{
		  $capacity_p_a="Above 80 Lakh BL";
		}else{
		  $capacity_p_a="";
		}
    ...
    <div style="text-align:center">
  	...
  	<table class="table table-bordered table-responsive">
								...
								<tr>
									<td width="50%">2.Name of Father/Husband </td>
									<td>'.strtoupper($name_father).'</td>
								</tr>
								<tr>
									<td width="50%">3.Sex of Applicant </td>
									<td>'.strtoupper($sex_applicant).'</td>
								</tr>
								<tr>
									<td width="50%">4.Age of the applicant </td>
									<td>'.strtoupper($owner_age).'</td>
								</tr>
								<tr>
									<td width="50%">4. Educational Qualification </td>
									<td>'.strtoupper($edu_quali).'</td>
								</tr>
							    <tr>
									<td width="50%" valign="top">5. Address of the Applicant </td>
									<td width="50%">
									<table class="table table-bordered table-responsive">
					...
					<tr>
							<td width="50%"> 6. Present and Past Occupation of the Applicant </td>
							<td>'.strtoupper($pre_past_occupation).'</td>
					</tr>
					<tr>
							<td width="50%"> 7. Whether the Applicant is a citizen of India as defined in articles 5 & 8 of the Constitution </td>
							<td>'.strtoupper($is_citizen).'</td>
					</tr>
					<tr>
							<td width="50%"> 8. Ever Convicted by a Criminal Court </td>
							<td>'.strtoupper($is_criminal).'</td>
					</tr>
					<tr>
							<td width="50%">9. Purposed  of the proposed plant </td>
							<td>'.strtoupper($proposed_plant).'</td>
					</tr>
					<tr>
						<td width="50%" valign="top">10. Proposed Site&#39;s Location</td>
						<td width="50%">
						<table class="table table-bordered table-responsive">
							<tr>
								  <td width="50%">Plot No. :</td>
					...
					<tr>
					      <td width="50%">11. Plant site </td>
						  <td>'.strtoupper($plant_site).'</td> 
					</tr>
					<tr>
					      <td width="50%">12. Apparatus Description </td>
					      <td>'.strtoupper($apparatus_description).'</td> 
					</tr>
					<tr>
					      <td width="50%">13. Tentative date </td>
						  <td>'.strtoupper($ten_date).'</td> 
					</tr>
					<tr>
					      <td width="50%">14. Estimated quantity </td>
					      <td>'.strtoupper($esti_quantity).'</td> 
					</tr>
					<tr>
					      <td width="50%">15. Production capacity </td>
						  <td>'.strtoupper($capacity_p_a).'</td> 
					</tr>
					<tr>
					      <td width="50%">16. Enviromental pollution,if </td>
					      <td>'.strtoupper($is_pollution).'</td> 
					</tr>
					<tr>
					      <td width="50%">17. Distance from Institutions </td>
						  <td>'.strtoupper($site_distance_a).'</td> 
					</tr>
					<tr>
					      <td width="50%">18. Distance from National/State Highway </td>
					      <td>'.strtoupper($site_distance_b).'</td> 
					</tr>
					<tr>
					      <td width="50%">19. Related to any Government servant,if </td>
