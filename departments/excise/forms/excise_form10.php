	...
	$p=$formFunctions->executeQuery($dept,"select * from ".$table_name." where user_id='$swr_id' and active='0' ORDER BY form_id DESC LIMIT 1");
		if($p->num_rows>0){
			...
			if(!empty($results["site_loc"])){
				...
				if(isset($site_loc->p)) $site_loc_p=$site_loc->p;    
				if(isset($site_loc->da)) $site_loc_da=$site_loc->da; 
				if(isset($site_loc->pt)) $site_loc_pt=$site_loc->pt; 
				if(isset($site_loc->d)) $site_loc_d=$site_loc->d;   
				if(isset($site_loc->ct)) $site_loc_ct=$site_loc->ct; 
				if(isset($site_loc->z)) $site_loc_z=$site_loc->z;    
			...
			$site_distance_a="";$site_distance_b="";$is_servant="";$capacity_p_a="";$is_pollution="";
		}
	}else{ 			
			$form_id=$results["form_id"];
			$name_father=$results["name_father"];$edu_quali=$results["edu_quali"];$owner_age=$results["owner_age"];
			...
			<div class="panel-body">
				...
				<form name="myform1" class="submit1" id="myform1" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
					<table id="" class="table table-responsive">
						...
						<tr>
						    <td>Email id</td>
							<td><input type="text" class="form-control" validate="email" disabled="disabled" value="<?php echo $email; ?>"></td>
							<td>State </td>
						...
						<tr>
							...
							<td>
							<label class="radio-inline"><input type="radio" name="is_criminal" class="is_criminal" value="Y"  <?php if(isset($is_criminal) && $is_criminal=='Y') echo 'checked'; ?> required="required" /> Yes</label>&nbsp;&nbsp;&nbsp;&nbsp;
							<label class="radio-inline"><input type="radio" class="is_criminal" name="is_criminal"  value="N"  <?php if(isset($is_criminal) && ($is_criminal=='N' || $is_criminal=='')) echo 'checked'; ?>/> No</label>
						    </td>
					   ...
					   <tr>
						...
						<tr>
						   ...
						   <td>15. Production capacity :</td>
							<td>
								<select name="capacity_p[a]" required="required" class="form-control text-uppercase">
										<option value="">Choose an option</option>
										<option value="A" <?php if($capacity_p_a=="A") echo "selected"; ?>>Upto 80 Lakh BL</option>
										<option value="B" <?php if($capacity_p_a=="B") echo "selected"; ?>>Above 80 Lakh BL</option>
								</select>
						   </td>
					  </tr>
					  <tr>
