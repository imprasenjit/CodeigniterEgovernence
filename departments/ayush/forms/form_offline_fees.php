...
if(isset($_GET["token"]) && is_numeric($_GET["token"]) && $_GET["token"]>0  && $_GET["token"]<3 ){
	$token=$_GET["token"];
	$check=$ayush->query("select b.process_type from ayush_form".$token." a,ayush_form".$token."_process b,ayush_form".$token."_certificates c where a.user_id='$swr_id' and b.form_id=a.form_id and c.form_id=a.form_id and b.process_type='A' and c.is_paid!='Y'") or die($ayush->error);
	if($check->num_rows>0){
		$form_id=$ayush->query("select form_id from ayush_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->form_id;
		$fees=$ayush->query("select total_fees from ayush_form".$token."_certificates where form_id='$form_id' and is_paid!='Y'")->fetch_object()->total_fees;
		$prev="../index.php";
	}else{
		die();
		echo "<script>
				alert('Something went wrong!!!');
				window.location.href = '".$server_url."departments/ayush/';
		</script>";
	}
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from ayush_form_names where form_no='1'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
										...
										<div id="" style="display: block;">
											<div class="row">
												<div class="col-md-6 col-md-offset-4">
													...
													<p>Head of Account :<br/>0210- Medical and Public Health ,04- Public Health ,104- Fees and Fines etc. </p>
