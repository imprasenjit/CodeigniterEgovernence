...
if(isset($_GET["token"])){
	$token=$_GET["token"];
	$check=$formFunctions->is_already_registered('ayush',$token);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=ayush';
			</script>";	
	}else if($check==0){
		echo "<script>
		...
		echo "<script>
				window.location.href = '".$server_url."departments/ayush/forms/ayush_form".$token.".php';
		</script>";
	}else{
		$submit_payment=$token;
		$reg_fees=$ayush->query("select reg_fees from ayush_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->reg_fees;
	}
}else{
	echo "<script>
			alert('Something went wrong!!!');
											...
											<div class="row">
												<div class="col-md-4 col-md-offset-4 box box-shadow" style="width:30%;padding-bottom:30px">
													<p><b><u>Challan Head</u></b><br/>
														0210- Medical and Public Health ,<br/>04- Public Health ,<br/>104- Fees and Fines etc. <br/><br/>
													</p>
													<div class="row">
