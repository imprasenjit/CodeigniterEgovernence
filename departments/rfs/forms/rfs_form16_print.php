...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	if($q->num_rows>0){	
		...
		if($firm_duration=="U"){
			$firm_duration="Unlimited";
		}else if($firm_duration=="L"){
			$firm_duration="Limited";
		}else{
			$firm_duration="Not Selected";
		}
		if($is_different=="Y"){
			$is_different="Yes";
		}else{
			...
			<tr>
				...
				<table class="table table-bordered table-responsive">
					...
					while($rows=$results1->fetch_object()){
						...
						if($rows->upload_signature !="") $upload_signature='<img style="padding:5px" width="150" height="60" src="'.$server_url.'departments/rfs/forms/upload/'.$rows->upload_signature .'"/>';
						...
						<tr>
							...
							<td>'.strtoupper($rows->member_name1).'</td>
							<td>'.strtoupper($rows->member_address1).'</td>
							<td>'.$upload_photo .'</td>
