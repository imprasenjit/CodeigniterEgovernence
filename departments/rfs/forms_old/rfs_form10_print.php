<?php if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$rfs->query("select * from rfs_form10 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$rfs->query("select * from rfs_form10 where uain='$uain' and user_id='$swr_id'") or die($rfs->error);
	}else if(isset($form_id)){
		$form_id=$form_id;
		$q=$rfs->query("select * from rfs_form10 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);		
	}else{
		$q=$rfs->query("select * from rfs_form10 where user_id='$swr_id' and active='1'") or die($rfs->error);
	}
	if(isset($css)){
	...
	$form_name=$formFunctions->get_formName('rfs','10');
