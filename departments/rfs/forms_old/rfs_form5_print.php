<?php if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$rfs->query("select * from rfs_form5where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$rfs->query("select * from rfs_form5 where uain='$uain' and user_id='$swr_id'") or die($rfs->error);
	}else if(isset($form_id)){
		$form_id=$form_id;
		$q=$rfs->query("select * from rfs_form5 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);		
	}else{
		$q=$rfs->query("select * from rfs_form5 where user_id='$swr_id' and active='1'") or die($rfs->error);
	}
	if(isset($css)){
	...
	if($previous_details->num_rows>0){
		...
		if($q->num_rows<1){	 ###EMPTY FORM DETAILS###
			$form_id="";$post_office="";$police_station="";$deed="";$date="";$place_deed="";
		}else{		
			$form_id=$results['form_id'];$post_office=$results['post_office'];$police_station=$results['police_station'];$reg_date=$results['reg_date'];
			$reg_no=$results['reg_no'];	$uain=$results['uain'];$deed=$results['deed'];$date=$results['date'];$place_deed=$results['place_deed'];	
			$file1=$results['file1'];$file2=$results['file2'];$file3=$results['file3'];$file4=$results['file4'];$file5=$results['file5'];$file6=$results['file6'];	
	...
	$form_name=$formFunctions->get_formName('rfs','8');
...
if(!isset($css)){
		...
		<table width="99%" align="center" class="table table-bordered table-responsive" style="margin:0px auto;border-collapse: collapse" border="1" >			
			...
			<tr>
				<td>3. Place of Deed Registration :</td>
			...
			<tr>
				<td>1. Form No. VI signed by the partners and witnessed by either a Judicial Magistrate or a Chartered Accountant :</td>
				<td>'.$val1.'</td> 
			</tr>
			<tr>
				<td>2. Registration Certificate of Firm: </td>
				<td>'.$val2.'</td> 
			</tr>
			<tr>
				<td>3. Certified copy of Registration Deed of Dissolution :</td>
				<td>'.$val3.'</td> 
			</tr>
