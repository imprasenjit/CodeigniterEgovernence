<?php if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$rfs->query("select * from rfs_form9 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$rfs->query("select * from rfs_form9 where uain='$uain' and user_id='$swr_id'") or die($rfs->error);
	}else if(isset($form_id)){
		$form_id=$form_id;
		$q=$rfs->query("select * from rfs_form9 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);		
	}else{
		$q=$rfs->query("select * from rfs_form9 where user_id='$swr_id' and active='1'") or die($rfs->error);
	}
	if(isset($css)){
	...
	if($previous_details->num_rows>0){
		...
		if($q->num_rows<1){	 ###EMPTY FORM DETAILS###
			$form_id="";$file1="";$file2="";$file3="";$file4="";$file5="";$file6="";$file7="";$file8="";
		}else{		
	...
	$form_name=$formFunctions->get_formName('rfs','9');
...
if(!isset($css)){
		...
		<div style="text-align:center">
  			'.$assamSarkarLogo.'<h4>Form No 8</h4>
  			<h4>'.$form_name.'</h4>
		</div><br/>
		...
		<tr>
			...
			<tr>
				<td>1.Photocopy of Registration Certificate of society: </td>
				<td>'.$val1.'</td> 
			</tr>
			<tr>
			...
			<tr>
				<td>3. A list showing the name,address,occupation and designation of Executive Committee entrusted with the management of the affairs of the society: </td>
				<td>'.$val3.'</td> 
			</tr>
			<tr>
			...
			<tr>
				<td>5. An application for filling changes in the balance sheet and Auditors Report of the society under section 4B of the S.R. Act XXI 1860:</td>
				<td>'.$val6.'</td> 
			</tr>
			<tr>
				<td>5. Within 30 days after holding of every Annual General Meeting there shall be filed  with the Registrar of Societies a copy of each of the balance sheet and Auditors Report: </td>
				<td>'.$val7.'</td> 
			</tr>
			<tr>
				<td>5. Resolution regarding acceptance and approval of the balance sheet and Auditors Report of the society under section 4B of the S.R. Act XXI 1860:</td>
				<td>'.$val8.'</td> 
			</tr>
			<tr>';
