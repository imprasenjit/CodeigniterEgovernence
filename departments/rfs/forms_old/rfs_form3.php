...
$check=$formFunctions->is_already_registered('rfs','3');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=3&dept=rfs';
		</script>";
}else{
	$showtab="";
}
...
$email=$formFunctions->get_usermail($swr_id);
	...
	if($previous_details->num_rows>0){
		...
		$q=$rfs->query("select * from rfs_form3 where user_id='$swr_id' and active='1'");
		$results=$q->fetch_assoc();
		
		if($q->num_rows<1){	 ###EMPTY FORM DETAILS###
			$form_id="";$file1="";$file2="";$file3="";$file4="";$file5="";$file6="";$file7="";$file8="";$file9="";$file10="";
		}else{		
		...
		if($q->num_rows>0){		
		    ...
		    $b_village=$results["b_village"];$b_postoffice=$results["b_postoffice"];$b_policestation=$results["b_policestation"];$b_district=$results["b_district"];$b_pincode1=$results["b_pincode1"];$b_mobile=$results["b_mobile"];$b_email1=$results["b_email1"];	$deed=$results["deed"];$date=$results["date"];$place_deed=$results["place_deed"];$deed1=$results["deed1"];$date1=$results["date1"];$place_deed1=$results["place_deed1"];$certificate=$results["certificate"];$issue=$results["issue"];$date_issue=$results["date_issue"];
			$file1=$results['file1'];$file2=$results['file2'];$file3=$results['file3'];$file4=$results['file4'];$file5=$results['file5'];$file6=$results['file6'];
			$file7=$results['file7'];$file8=$results['file8'];$file9=$results['file9'];
			$file10=$results['file10'];
		}		
	}
	##PHP TAB management	
...
<body class="hold-transition skin-blue fixed" data-target="#scrollspy" data-spy="scroll">
	<div class="wrapper">
	  ...
	  <?php require '../../../user_area/includes/aside.php'; ?>
		<!-- Content Wrapper. Contains page content -->
				...
				<section class="content">
						...
						<div class="col-md-12">
							<div class="panel panel-primary">
								<div class="panel-heading">
									...
									<strong>Form No III<br/><?php echo $form_name=$cms-> query("select form_name from rfs_form_names  where  form_no='3'")->fetch_object()->form_name; ?></strong>
								</h4>	
							    </div>
                         ...
                         <table class="table table-responsive">
						       <tr>
							    <td> Closing  place of business : </td>
					           <td> <span class="soc_alert"></span> </td>
				            </tr>
						    <tr>
							  <td> Date :</td> <td><input type="text" class="date_picker form-control"  name="est_date" readonly="readonly" disabled value="<?php if(isset($date_of_commencement)){echo $date_of_commencement;} ?>" required /></td>
					...
					<tr>
					 ...
					 <tr>
							    <td> Opening place of business : </td>
					           <td> <span class="soc_alert"></span> </td>
				            </tr>
						    <tr>
						      <td> Date :</td><td><input type="text" class="date_picker form-control" name="est_date" readonly="readonly" disabled value="<?php if(isset($date_of_commencement)){echo $date_of_commencement;} ?>" required />
					          <td> Locality : </td><td><input type="text"  class="form-control text-uppercase" disabled value="<?php echo 	$b_street_name2; ?>" ></td>
						    </tr> 
					...
					<tr>
						 <td>Pin code :</td><td><input type="text"  class="form-control text-uppercase" disabled value="<?php echo $b_pincode; ?>" ></td>
						    ...
						    <tr>
						      <td> Deed No.:</td><td><input type="text"  class="form-control"  value="<?php echo $deed; ?>" ></td>
					          <td> Date : </td><td><input type="text"  class="date_picker form-control"  value="<?php echo 	$date; ?>" ></td>
						    </tr> 
					        <tr>
							   <td>Place of Deed Registration:</td><td><input type="text"  class="form-control text-uppercase"  value="<?php echo 	$place_deed; ?>" ></td>
                          </tr>	
						   <tr>
						    ...
						    <tr>
						      <td> Deed No.:</td><td><input type="text"  class="form-control"  value="<?php echo $deed1; ?>" ></td>
					          <td> Date : </td><td><input type="text"  class="date_picker form-control"  value="<?php echo 	$date1; ?>" ></td>
						    </tr> 
					        <tr>
							   <td>Place of Deed Registration:</td><td><input type="text"  class="form-control text-uppercase" value="<?php echo 	$place_deed1; ?>" ></td>
                          </tr>
                          <tr>
                          <td>Photos of the partners:
                           </td>
						    ...
						    <tr>
						      <td>Certificate No.:</td><td><input type="text"  class="form-control" value="<?php echo $certificate; ?>" ></td>
					          <td> Issued by : </td><td><input type="text"  class="form-control text-uppercase" value="<?php echo 	$issue; ?>" ></td>
						        <td> Date of issue : </td><td><input type="text"  class="date_picker form-control" value="<?php echo 	$date_issue; ?>" ></td>
						   </tr> 
					...
					<tr>
					  ...
					  <button type="submit" class="btn btn-success" name="save3" title="Save it and fill up the form later and Go to the Next Part"  onclick="return confirm('Do you want to save..?')" >Save and Next</button>
					  </td>
		    ...
		    <tr>
		    <td>Form No. III signed by the partners and witnessed by either a judicial Magistrate or a Chartered Accountant .  </td>
		...
		<tr>
					<td>Land document of Place of Business. If not rent agreement or Affidavit from house owner: </td>
				...
				<tr>
					 <td>Trade license,obtainable from the Municipal Corporation/Municipal Board /Town Committee or Goan Panchayat: </td>
				...
				<tr>
					 <td>General Index Register Number(GIR number): </td>
				...
				<tr>
					 <td>Sales Tax or Income Tax if not an affidavit of the same:  </td>
					      <td><select trigger="FileModal" class="file9" id="file9" <?php if($file9!="" || $file9=="SC" || $file9=="NA") echo "disabled='disabled'"; ?>>
							<option value="0" selected="selected">Select</option>
							<option value="1">From E-Locker</option>
							<option value="2">From PC</option>
						</select>
					<input type="hidden" name="mfile9" value="<?php if($file9!="") echo $file6; ?>" id="mfile9" readonly="readonly"/></td>
			  ...
			  <tr>
					<td class="text-center" colspan="4">
						<a href="rfs_form3.php?tab=2" class="btn btn-primary">Go Back & Edit</a>										
