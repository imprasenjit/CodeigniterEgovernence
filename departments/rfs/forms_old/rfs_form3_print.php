<?php if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$rfs->query("select * from rfs_form3 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$rfs->query("select * from rfs_form3 where uain='$uain' and user_id='$swr_id'") or die($rfs->error);
	}else if(isset($form_id)){
		$form_id=$form_id;
		$q=$rfs->query("select * from rfs_form3 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);		
	}else{
		$q=$rfs->query("select * from rfs_form3 where user_id='$swr_id' and active='1'") or die($rfs->error);
	}
	if(isset($css)){
	...
	if($previous_details->num_rows>0){
		...
		if($q->num_rows<1){	 ###EMPTY FORM DETAILS###
			...
			$file1=$results['file1'];$file2=$results['file2'];$file3=$results['file3'];$file4=$results['file4'];$file5=$results['file5'];$file6=$results['file6'];	$file6=$results['file6'];	$file7=$results['file7'];$file8=$results['file8'];$file9=$results['file9'];$file10=$results['file10'];
		}
	}else{
		$uain="";$reg_date="";
	...
	if(!isset($css)){
		...
		$val6=$formFunctions->get_uploadFile($file6,$applicant_id);
	    $val7=$formFunctions->get_uploadFile($file7,$applicant_id);
		$val8=$formFunctions->get_uploadFile($file8,$applicant_id);
		$val9=$formFunctions->get_uploadFile($file9,$applicant_id);
		$val10=$formFunctions->get_uploadFile($file10,$applicant_id);
	}	
	$form_name=$formFunctions->get_formName('rfs','3');
...
if(!isset($css)){
		...
		<table width="99%" align="center" class="table table-bordered table-responsive" style="margin:0px auto;border-collapse: collapse" border="1" >	
			...
			<tr>
				...
				<td>'.strtoupper($certificate).'</td>
			</tr>	
			<tr>
				<td>Issued by : </td>
				<td>'.strtoupper($issue).'</td>
			</tr>
            <tr>
				<td>Date of issue : </td>
				<td>'.date("d-m-Y",strtotime($date_issue).'</td>
			</tr>			
		...
		<tr>
			...
			<tr>
				<td>1.Registration Certificate of Firm:</td>
				<td>'.$val1.'</td> 
			</tr>
			<tr>
				<td>2. Form No. III signed by the partners and witnessed by either a judicial Magistrate or a Chartered Accountant . </td>
				<td>'.$val2.'</td> 
			</tr>
			<tr>
			...
			<tr>
				<td>5. Land document of Place of Business. If not rent agreement or Affidavit from house owner: </td>
				<td>'.$val5.'</td> 
			</tr>
			<tr>
				<td>6. Trade license,obtainable from the Municipal Corporation/Municipal Board /Town Committee or Goan Panchayat: </td>
				<td>'.$val6.'</td> 
			</tr>
			<tr>
				<td>7. PAN Card:  </td>
				...
				<tr>
				<td>8. General Index Register Number(GIR number):   </td>
				<td>'.$val8.'</td> 
			</tr>
				<tr>
				<td>9. Sales Tax or Income Tax if not an affidavit of the same:    </td>
				<td>'.$val9.'</td> 
			</tr>
				<tr>
				<td>10. Treasury Challan:    </td>
				<td>'.$val10.'</td> 
			</tr>
