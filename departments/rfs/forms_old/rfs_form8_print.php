<?php if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$rfs->query("select * from rfs_form8 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$rfs->query("select * from rfs_form8 where uain='$uain' and user_id='$swr_id'") or die($rfs->error);
	}else if(isset($form_id)){
		$form_id=$form_id;
		$q=$rfs->query("select * from rfs_form8 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);		
	}else{
		$q=$rfs->query("select * from rfs_form8 where user_id='$swr_id' and active='1'") or die($rfs->error);
	}
	if(isset($css)){
...
if(!isset($css)){
		...
		<table width="99%" align="center" class="table table-bordered table-responsive" style="margin:0px auto;border-collapse: collapse" border="1" >			
			...
			<tr>
			    <td>Mouza</td>
				<td>'.strtoupper(	$b_mouza).'</td>
			</tr>
			<tr>
				<td>Circle</td>
				<td>'.strtoupper($b_circle).'</td>
			</tr>
			<tr>
				<td>Patta no</td>
				<td>'.strtoupper($b_patta).'</td>
			</tr>
			<tr>
				<td>Dag no</td>
				<td>'.strtoupper(	$b_dag).'</td>
			</tr>
			<tr>
				<td>Area</td>
				<td>'.strtoupper($b_area).'</td>
			</tr>
			<tr>
				<td>Locality</td>
				<td>'.strtoupper($b_locality).'</td>
			</tr>
			<tr>
				<td>Village</td>
				<td>'.strtoupper($b_village).'</td>
			</tr>
			<tr>
				<td>Post Office</td>
				<td>'.strtoupper(	$b_postoffice).'</td>
			</tr>
			<tr>
				<td>Police Station</td>
				<td>'.strtoupper($b_policestation).'</td>
			</tr>
			<tr>
				<td>District</td>
				<td>'.strtoupper($b_district).'</td>
			</tr>
			<tr>
				<td>Pin code </td>
				<td>'.strtoupper($b_pincode1).'</td>
			</tr>
			<tr>
				<td>Mobile No.</td>
				<td>'.strtoupper($b_mobile).'</td>
			</tr>
			<tr>
				<td>Email ID</td>
				<td>'.$b_email1.'</td>
		...
		<tr>
			...
			<tr>
				<td>4. Affidavit regar
				ding change of address of the society.</td>
				<td>'.$val4.'</td> 
			</tr>
			<tr>
