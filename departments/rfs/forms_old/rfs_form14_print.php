<?php if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$rfs->query("select * from rfs_form14 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$rfs->query("select * from rfs_form14 where uain='$uain' and user_id='$swr_id'") or die($rfs->error);
	}else if(isset($form_id)){
		$form_id=$form_id;
		$q=$rfs->query("select * from rfs_form14 where user_id='$swr_id' and form_id='$form_id'") or die($rfs->error);		
	}else{
		$q=$rfs->query("select * from rfs_form14 where user_id='$swr_id' and active='1'") or die($rfs->error);
	}
	if(isset($css)){
	...
	if($previous_details->num_rows>0){
		...
		$q=$rfs->query("select * from rfs_form14 where user_id='$swr_id' and active='1'");
	...
	$form_name=$formFunctions->get_formName('rfs','14');
...
if(!isset($css)){
		...
		<tr>
			...
			<tr>
				<td>2. An application for approval of amendment of Memorandum of Association and constitution of the society.</td>
				<td>'.$val2.'</td> 
			</tr>
			<tr>
				<td>3. A copy of notice regarding special meeting for the proposed amendments is to be issued to all members of the society  10 days before the special meeting</td>
				<td>'.$val3.'</td> 
			</tr>
			<tr>
				<td>4. The proposal amendments shall have to be agreed to by the votes of three-fifth of the members in proxy .</td>
				<td>'.$val4.'</td> 
			</tr>
			<tr>
				<td>5. The proposed amendments  shall have to be confirmed by votes of three-fifth of the members present at a second special meeting convened by the governing body at an interval of one month after the former meeting </td>
				<td>'.$val5.'</td> 
			</tr>
			<tr>
				<td>6. Present Rules and Regulation of the society.</td>
				<td>'.$val6.'</td> 
			</tr>';
			if(!empty($results["courier_details"]) && $results["courier_details"]!=1){
		$printContents=$printContents.'
