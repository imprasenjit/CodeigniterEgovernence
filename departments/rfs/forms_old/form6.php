...
$sql=$rfs->query("select * from rfs_form1 where user_id='$swr_id' ");	
	...
	$sq=$rfs->query("select * from rfs_form6 where user_id='$swr_id' ");
        $ro=$sq->fetch_array();	
	...
	<!-- Content Wrapper. Contains page content -->
			<div class="content-wrapper">
				...
				<section class="content">
					...
					<div class="row">
                        ...
                        <strong> 
                        Form No V<br>
                        Intimation for recording of Changes in Constitution of a Firm<br>
                        The Indian Partnership Act. 1932,(See Section 63(I) and Rules 4(6))<br>
                        Filling Fee : Rs. 1.00
                   </strong>
                    </h4>	
								</div>
			...
			<form name="myform1" id="myform1" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
                ...
                <tr>
                    <td colspan="2">
                        <p>
                            Notice is hereby given, pursuant to sub-section (I) of Section 63 and Rule 4(6) of the Indian Partnership Act,1932 of the following changes in the Constitution of the firm M/S
                            <input type="text"  style="width:150px;" id="firm_name"  class="form-control" name="firm_name" pattern="[a-zA-Z_/.\s]+$" value="<?php echo $firm_name;?>" required/>
			 ...
			 <thead>
			 <tr>
                    <th colspan="3">PREVIOUS CONSTITUTION OF THE FIRM</th>
                    <th colspan="3">PRESENT CONSTITUTION OF THE FIRM</th>
                </tr>
				       <th></th>
					 <th>Name of Partners</th>
                    <th>Permanent Addresses</th>
                    <th>Names of Partners</th>
                    <th>Permanent Addresses</th>
                    <th>Remarks</th>
				...
				if($memberCount>0){
					...
					for($m=1;$m<=count($partner_address);$m++){					
						...
						<td><?php echo ($m); ?></td>
						<td>
						<input type="text" validate="specialChar" class="form-control text-uppercase"name="partner_address[<?php echo $m; ?>][name]" pattern="[a-zA-Z_/.\s]+$" value="<?php echo $partner_address[$m]['name']; ?>" required/>
						</td>
                       <td>
						<input type="text" validate="specialChar" class="form-control text-uppercase" name="partner_address[<?php echo $m; ?>][address]" pattern="[a-zA-Z_/.\s]+$" value="<?php echo $partner_address[$m]['address']; ?>" required/>
						</td>
                         <td>
						<input type="text" validate="specialChar" class="form-control text-uppercase" name="partner_address[<?php echo $m; ?>][present_name]" pattern="[a-zA-Z_/.\s]+$" value="<?php echo $partner_address[$m]['present_name']; ?>" required/>
						  ...
						  <td>
						<input type="text" validate="specialChar" class="form-control text-uppercase" name="partner_address[<?php echo $m; ?>][present_address]" pattern="[a-zA-Z_/.\s]+$" value="<?php echo $partner_address[$m]['present_address']; ?>" required/>
						</td>						
                         <td>
				...
				?>
					<tr><td>1</td>
                    <td><input type="text"  pattern="[a-zA-Z_/.\s]+$" name="partner_address[1][name]" class="form-control text-uppercase" required/></td>
                    <td><input type="text" pattern="[a-zA-Z_/.\s]+$" name="partner_address[1][address]" class="form-control text-uppercase" required/></td>
					<td><input type="text" pattern="[a-zA-Z_/.\s]+$" name="partner_address[1][present_name]" class="form-control text-uppercase" required/></td>
					<td><input type="text" pattern="[a-zA-Z_/.\s]+$" name="partner_address[1][present_address]" class="form-control text-uppercase" required/></td>
				...
				<?php  } ?>
	             <tr id="sumit" colspan="6">&nbsp;</tr>
					</tbody>
				</table>
				 <table><tr>
						<td><a class="memberBtn" jsTag="ADD">ADD MORE</a>&nbsp;&nbsp;
						<input id="index" type="hidden" name="hidden" value="<?php echo $moreindexvalex; ?>"/>
						&nbsp;&nbsp;<td style="display:none;" id="DEL"><a   class="memberBtn"  jsTag="DELETE">DELETE</a></td>
                    </tr>
					</table>
            ...
            <p style="text-align:center">
                   <button type="submit" style="font-weight:bold" name="save6" class="btn btn-primary">Save </button>
               <button type="submit" class="btn btn-success" name="submit6" title="Save it and fill up the form later and Go to the Next Part" onclick="return confirm('Do you want to save..?')"> SUBMIT</button>
                </p>
        </form>
       </div>
			</div>
