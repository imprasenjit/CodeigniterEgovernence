        ...
        var imageUpload = {
            ...
            handlebuttonevents: function() {
                $(document).on("change", this.obj.selector, function() {
                    imageUpload.checkFile(imageUpload.obj[0].files[0]);
                });
                $(document).on('click','.pkdel',function(){
                    var parent = $(this).parent('div').parent('div');
                    imageUpload.delAndRearrange(parent);
                });
            },
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 1 || data.success == 1) {
                            ...
                            if (options.showErrorAlerts) {
                                imageUpload.addWarning(err, $('div.row[rel="' + pos + '"]'));
                            }
                            $('div.row[rel="' + pos + '"]').remove();
                            imageUpload.hasErrors = true;
                    ...
                    error: function(xhr, ajaxOptions, thrownError) {
                        imageUpload.files.splice(pos, 1);
                        if (options.showErrorAlerts) {
                            imageUpload.addWarning(thrownError, $('div.pkrw[rel="' + pos + '"]'));
                        }
                        imageUpload.hasErrors = true;
                    ...
                    xhr: function() {
                        ...
                        if (myXhr.upload) {
                            myXhr.upload.addEventListener("progress", function(e) {
                                imageUpload.handleProgress(e, pos);
                            }, false);
                        }
                        return myXhr;
                    },
                    complete: function() {
                        if (options.onSubmit) {
                            imageUpload.uploadedfiles++;
                            if (imageUpload.uploadedfiles == imageUpload.files.length && imageUpload.hasErrors == false) {
                                imageUpload.obj.remove();
                                imageUpload.obj.parent("form").submit();
                            }
                        }
                    },
                    cache: false,
            ...
            handleFormSubmission: function() {
                ...
                form.submit(function() {
                    imageUpload.hasErrors = false;
                    imageUpload.uploadedfiles = 0;
                    for (var i = 0; i < imageUpload.files.length; i++) {
                        if (imageUpload.files[i]) {
                            imageUpload.upload(imageUpload.files[i], i);
                        } else {
                            imageUpload.uploadedfiles++;
                            if (imageUpload.uploadedfiles == imageUpload.files.length && imageUpload.hasErrors == false) {
                                imageUpload.obj.remove();
            ...
            delAndRearrange: function(parent){
                ...
                imageUpload.files.splice(parseInt(id), 1);
                parent.remove();
                imageUpload.container.find('div.pkrw').each(function(index){
                    $(this).attr('rel',index);
                });
            }
        };
