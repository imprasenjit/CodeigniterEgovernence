...
$save_mode="D";
if($token=="trade"){
	$check=$formFunctions->is_already_registered('dma','1');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=1&dept=dma';
			</script>";	
	}
	$form_name="APPLICATION FOR TRADE LICENSE";
	$sql=$dma->query("select form_id,uain,save_mode from dma_form1 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$check=$formFunctions->is_already_registered('dma','2');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=2&dept=dma';
			</script>";	
	}
	$form_name="APPLICATION FOR INDUSTRIAL TRADE LICENSE";
	$sql=$dma->query("select form_id,uain,save_mode from dma_form2 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$check=$formFunctions->is_already_registered('dma','3');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=3&dept=dma';
			</script>";	
	}
	$form_name="APPLICATION FOR HEALTH LICENSE";
	$sql=$dma->query("select form_id,uain,save_mode from dma_form3 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$check=$formFunctions->is_already_registered('dma','4');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=4&dept=dma';
			</script>";	
	}
	$form_name="APPLICATION FOR VETERINARY LICENSE";
	$sql=$dma->query("select form_id,uain,save_mode from dma_form4 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
...
$returnUrl=$server_url."departments/dma/forms/form_payment.php?token=".$token;
...
if($sid==18){
$newData="DOIACGOA|".$sentCustomerID."|NA|1.00|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|dma001|".$txDateTime."|".$form_name."|NA|NA|".$returnUrl;
}else{
$newData="DOIACGOA|".$sentCustomerID."|NA|100.00|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|dma001|".$txDateTime."|".$form_name."|NA|NA|".$returnUrl;
}
...
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=1&dept=dma" />
<?php }
	if($formName=="F2"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=2&dept=dma" />
<?php } 
	if($formName=="F3"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=3&dept=dma" />
<?php } 
	if($formName=="F4"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=4&dept=dma" />
<?php } ?>  
<?php } ?> 
</head>
