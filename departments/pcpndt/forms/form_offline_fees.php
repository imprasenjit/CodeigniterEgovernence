...
if(isset($_GET["token"]) && $_GET["token"]=="trade"){
	$check=$dma->query("select process_type from dma_form1 a,dma_form1_process b,dma_form1_certificates c where a.user_id='$swr_id' and b.form_id=a.form_id and b.process_type='A' and c.is_paid!='Y'") or die($dma->error);
	if($check->num_rows>0)
	{
		$form_id=$dma->query("select form_id from dma_form1 where user_id='$swr_id' order by form_id desc LIMIT 1 ")->fetch_object()->form_id;
		$fees=$dma->query("select total_fees from dma_form1_certificates where form_id='$form_id' and is_paid!='Y'")->fetch_object()->total_fees;		
		$prev="../index.php";
	}
