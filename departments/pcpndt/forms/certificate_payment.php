...
$is_paid="N";
if($token=="trade"){
	$formName=$cms->query("select form_name from dma_form_names where form_no='1'")->fetch_object()->form_name;
	$sql=$dma->query("select form_id,uain from dma_form1 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uian=$row['uain'];
		#certificate check
		$sqlC=$dma->query("select total_fees,is_paid from dma_form1_certificates where form_id='$form_id'") or die("Error : ".$dma->error);
...
$returnUrl=$server_url."dma/certificate_payment.php?token=".$token;
...
$newData="DOIACGOA|".$sentCustomerID."|NA|".$certificate_fees."|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|dma001|".$txDateTime."|".$formName."|NA|NA|".$returnUrl;
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from dma_form_names where form_no='1'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
