...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$clm->query("select * from clm_form4 where user_id='$swr_id' and form_id='$form_id'") or die($clm->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$clm->query("select * from clm_form4 where uain='$uain' and user_id='$swr_id'") or die($clm->error);
	}else if(isset($form_id)){
		$form_id=$form_id;
		$q=$clm->query("select * from clm_form4 where user_id='$swr_id' and form_id='$form_id'") or die($clm->error);
	}else{
		$q=$clm->query("select * from clm_form4 where user_id='$swr_id' and active='1'") or die($clm->error);
	}
	...
	$form_name=$formFunctions->get_formName('clm','4');
	$assamSarkarLogo=$formFunctions->getAssamSarkarLogo($server_url);
if(!isset($css)){
		$printContents='<!DOCTYPE html>
		<html lang="en">
		<head>
		<title>Form LM-II</title>
		<style type="test/css">
		table, thead, td {
		...
		<div style="text-align:center">
  			'.$assamSarkarLogo.'<h4>Form LM-II<br/>SCHEDULE II B<br/>[See rule 11 (2)]<br/>'.$form_name.'</h4>
		</div><br/>
	...
	<tr>
		<td colspan="2">
			<table width="100%" align="center" class="table table-bordered table-responsive" style="border-collapse: collapse" border="1">
					...
					$results1=$clm->query("select * from clm_form4_members where form_id='$form_id'") or die("Error : ".$clm->error);
					$sl=1;
