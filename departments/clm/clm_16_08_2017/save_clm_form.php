...
if(isset($_POST["save1"])){		
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
			$query=$clm->query("insert into clm_form1(user_id,sub_date,nature,fact,monogram,tools,workshop,facilities,elect_energy,is_loan_detail,bankers,reg_number,total_turnover,is_applied,is_applied_details,is_proposed,approval,inspection,type,persons) values ('$swr_id','$today', '$nature', '$fact', '$monogram','$tools', '$workshop', '$facilities', '$elect_energy','$is_loan_detail','$bankers','$reg_number','$total_turnover','$is_applied','$is_applied_details','$is_proposed','$approval','$inspection','$type','$persons')") OR die("Error: ".$clm->error);
		...
		$form_id=$row["form_id"];	
		$query=$clm->query("update clm_form1 set sub_date='$today', nature='$nature', fact='$fact', monogram='$monogram',tools='$tools',workshop='$workshop',facilities='$facilities',elect_energy='$elect_energy', is_loan_detail='$is_loan_detail',bankers='$bankers',reg_number='$reg_number',total_turnover='$total_turnover',is_applied='$is_applied', is_applied_details='$is_applied_details', is_proposed='$is_proposed',approval='$approval', inspection='$inspection' , type='$type',persons='$persons' where form_id=$form_id") OR die("Error: ".$clm->error);
		for($i=1;$i<=$hidden_value;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
...
if(isset($_POST["save2"])){		
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
			$query=$clm->query("insert into clm_form2(user_id,sub_date,is_lease_doc,fact,area,previous,machinery,elect_energy,sufficient,reg_number,total_turnover,is_applied,is_applied_details,weights_measure,persons) values ('$swr_id','$today', '$is_lease_doc', '$fact', '$area','$previous', '$machinery','$elect_energy','$sufficient','$reg_number','$total_turnover','$is_applied','$is_applied_details','$weights_measure','$persons')") OR die("Error:".$clm->error);
			$form_id=$clm->insert_id;
			for($i=1;$i<=$hidden_value;$i++){ 
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
		...
		$form_id=$row["form_id"];	
		$query=$clm->query("update clm_form2 set sub_date='$today', is_lease_doc='$is_lease_doc', fact='$fact', area='$area',previous='$previous', machinery='$machinery', elect_energy='$elect_energy', sufficient='$sufficient', reg_number='$reg_number',total_turnover='$total_turnover',is_applied='$is_applied', is_applied_details='$is_applied_details', weights_measure='$weights_measure', persons='$persons' where form_id=$form_id") OR die("Error: ".$clm->error);
		for($i=1;$i<=$hidden_value;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
...
if(isset($_POST["save3"])){		
	$weights_measure=clean($_POST["weights_measure"]);$reg_number=clean($_POST["reg_number"]);$is_intend=clean($_POST["is_intend"]);$is_applied=clean($_POST["is_applied"]);$total_turnover=clean($_POST["total_turnover"]);
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
			$query=$clm->query("insert into clm_form3(user_id,sub_date,fact,weights_measure,reg_number,total_turnover,is_intend,source_supply,monogram,lic_num,regis_impoter,model_impoter,is_applied,is_applied_details) values ('$swr_id','$today', '$fact', '$weights_measure','$reg_number','$total_turnover','$is_intend', '$source_supply','$monogram','$lic_num','$regis_impoter','$model_impoter','$is_applied','$is_applied_details')") OR die("Error: 1".$clm->error);
			$form_id=$clm->insert_id;
			for($i=1;$i<=$hidden_value;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
		...
		$form_id=$row["form_id"];	
		$query=$clm->query("update clm_form3 set sub_date='$today', fact='$fact',weights_measure='$weights_measure',reg_number='$reg_number',total_turnover='$total_turnover',is_intend='$is_intend', source_supply='$source_supply', monogram='$monogram', lic_num='$lic_num', regis_impoter='$regis_impoter',model_impoter='$model_impoter',  is_applied='$is_applied', is_applied_details='$is_applied_details' where form_id=$form_id") OR die("Error: ".$clm->error);
		for($i=1;$i<=$hidden_value;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
...
if(isset($_POST["save4"])){		
	...
	$tax_reg_no=clean($_POST["tax_reg_no"]);$state=clean($_POST["state"]);$license_fee=clean($_POST["license_fee"]);$license_fee_words=clean($_POST["license_fee_words"]);
	$license_fee_date=clean($_POST["license_fee_date"]);
	if($license_fee_date!=""){
		$license_fee_date=date("Y-m-d",strtotime($license_fee_date));
	}else{
		$license_fee_date=NULL;
	}		
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
			$query=$clm->query("insert into clm_form4(user_id,sub_date,license_no,type,type_changes,weight_trademark,workshop_details,production_details,shop_reg_no,shop_reg_date,tax_reg_no,total_turnover,state,license_fee,license_fee_words,license_fee_date) values ('$swr_id','$today', '$license_no', '$type', '$type_changes','$weight_trademark', '$workshop_details', '$production_details', '$shop_reg_no','$shop_reg_date','$tax_reg_no','$total_turnover','$state','$license_fee','$license_fee_words','$license_fee_date')") OR die("Error:".$clm->error);
			$form_id=$clm->insert_id;
		...
		$form_id=$row["form_id"];	
		$query=$clm->query("update clm_form4 set sub_date='$today', license_no='$license_no', type='$type', type_changes='$type_changes',weight_trademark='$weight_trademark', workshop_details='$workshop_details', production_details='$production_details', shop_reg_no='$shop_reg_no', shop_reg_date='$shop_reg_date', tax_reg_no='$tax_reg_no',  total_turnover='$total_turnover',state='$state', license_fee='$license_fee', license_fee_words='$license_fee_words', license_fee_date='$license_fee_date' where form_id=$form_id") OR die("Error: ".$clm->error);
		for($i=1;$i<=$input_size;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
...
if(isset($_POST["save5"])){		
	...
	$it_reg_no=clean($_POST["it_reg_no"]);$any_change=clean($_POST["any_change"]);$op_area=clean($_POST["op_area"]);$hav_u=clean($_POST["hav_u"]);$stamp_details=clean($_POST["stamp_details"]);$state=clean($_POST["state"]);$lic_fee=clean($_POST["lic_fee"]);$lic_fee_words=clean($_POST["lic_fee_words"]);
	$bank_sub_date=clean($_POST["bank_sub_date"]);
	if($bank_sub_date!=""){
		$bank_sub_date=date("Y-m-d",strtotime($bank_sub_date));
	}else{
		$bank_sub_date=NULL;
	}
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
			$query=$clm->query("insert into clm_form5(user_id,sub_date,repairer_lic,tl_reg_no,tl_date,it_reg_no,total_turnover,type_wm,any_change,op_area,hav_u,stamp_details,state,lic_fee,lic_fee_words,bank_sub_date) values ('$swr_id','$today', '$repairer_lic', '$tl_reg_no', '$tl_date','$it_reg_no','$total_turnover','$type_wm', '$any_change', '$op_area','$hav_u','$stamp_details','$state','$lic_fee','$lic_fee_words','$bank_sub_date')") OR die("Error:".$clm->error);
			$form_id=$clm->insert_id;
			for($i=1;$i<=$hidden_value;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
		...
		$form_id=$row["form_id"];	
		$query=$clm->query("update clm_form5 set sub_date='$today', repairer_lic='$repairer_lic', tl_reg_no='$tl_reg_no', tl_date='$tl_date', it_reg_no='$it_reg_no',total_turnover='$total_turnover',type_wm='$type_wm', any_change='$any_change', op_area='$op_area',hav_u='$hav_u',stamp_details='$stamp_details',state='$state',lic_fee='$lic_fee',lic_fee_words='$lic_fee_words',bank_sub_date='$bank_sub_date' where form_id=$form_id") OR die("Error: ".$clm->error);
		for($i=1;$i<=$hidden_value;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
...
if(isset($_POST["save6"])){ 	
	...
	$state=clean($_POST["state"]);$license_fee=clean($_POST["license_fee"]);$license_fee_words=clean($_POST["license_fee_words"]);
	...
	$input_size=clean($_POST["hidden_value"]);
	if($sql->num_rows<1){   ////////////table is empty//////////////				
			$query=$clm->query("insert into clm_form6(user_id,sub_date,license_no,date,reg_no,reg_date,categories,tax_reg,total_turnover,manu_details,state,license_fee,license_fee_words,license_fee_date) values ('$swr_id','$today', '$license_no', '$date', '$reg_no','$reg_date', '$categories', '$tax_reg','$total_turnover','$manu_details','$state','$license_fee','$license_fee_words','$license_fee_date')") OR die("Error:".$clm->error);
			$form_id=$clm->insert_id;
			for($i=1;$i<=$input_size;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
		...
		$form_id=$row["form_id"];	
		$query=$clm->query("update clm_form6 set sub_date='$today', license_no='$license_no',date='$date',reg_no='$reg_no',reg_date='$reg_date', categories='$categories',tax_reg='$tax_reg',total_turnover='$total_turnover',manu_details='$manu_details',state='$state', license_fee='$license_fee', license_fee_words='$license_fee_words', license_fee_date='$license_fee_date' where form_id=$form_id") OR die("Error: ".$clm->error);
		for($i=1;$i<=$input_size;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$pincode=$_POST["pincode".$i.""];$contact=$_POST["contact".$i.""];
...
if(isset($_POST["payment7"])){
	...
	if($query->num_rows<1){
		...
		if(empty($_POST["offline_challan"]) || $_POST["offline_challan"]=='2' || $_POST["offline_challan"]=='3'){
			...
			$update_result=$formFunctions->insert_offline_payment_details("clm",$uain,$reg_fees,$ref_no,$txn_date,$bank_name,"A");
			if($update_result==1){	
				...
				if($save_query){				
					...
					echo "<script>alert('Something went wrong !!!');window.location.href = 'payment_section.php?token=7';</script>";
				}
			}else{
				echo "<script>alert('Something went wrong !!!');window.location.href = 'payment_section.php?token=7';</script>";
			}
...
if(isset($_POST["save8"])){		
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
			$query=$clm->query("insert into clm_form8(user_id,sub_date,warehouse_addr,details_f_pack,imp_country) values ('$swr_id','$today', '$warehouse_addr', '$details_f_pack', '$imp_country')") OR die("Error:".$clm->error);
			$form_id=$clm->insert_id;
			for($i=1;$i<=$hidden_value;$i++){
				...
				$query1=$clm->query("INSERT INTO clm_form8_members(id,form_id,sl_no,name,family_name,address,mem_pincode,contact) VALUES ('','$form_id','$i','$name','$family_name','$address','$mem_pincode','$contact')") or die("error1 in insertion clm_form8_members".$clm->error);
				
			}
	}else{
		$form_id=$row["form_id"];	
		$query=$clm->query("update clm_form8 set sub_date='$today', warehouse_addr='$warehouse_addr', details_f_pack='$details_f_pack', imp_country='$imp_country' where form_id=$form_id") OR die("Error: ".$clm->error);
		for($i=1;$i<=$hidden_value;$i++){
				$name=$_POST["name".$i.""];$family_name=$_POST["family_name".$i.""];$address=$_POST["address".$i.""];$mem_pincode=$_POST["mem_pincode".$i.""];$contact=$_POST["contact".$i.""];
			
				$query1=$clm->query("update clm_form8_members set name='$name',family_name='$family_name',address='$address',mem_pincode='$mem_pincode',contact='$contact' where form_id='$form_id' and sl_no='$i'") or die("error in insertion clm_form8_members".$clm->error);
		}
	}				
	if($query){
