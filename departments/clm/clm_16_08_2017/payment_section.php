...
if(isset($_GET["token"])){
	...
	if($check==1){
		...
		echo "<script>
				window.location.href = '".$server_url."departments/clm/forms/clm_form".$token.".php';
		</script>";
	}else{
		
		$submit_payment=$token;
		$reg_fees=$clm->query("select reg_fees from clm_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->reg_fees;
	}
}else{
	die();
	echo "<script>
