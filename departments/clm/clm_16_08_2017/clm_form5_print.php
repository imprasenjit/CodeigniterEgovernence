...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
    ...
    $row1=$formFunctions->fetch_swr($swr_id);
        ...
        if($q->num_rows>0){
			...
			$it_reg_no=$results["it_reg_no"];$type_wm=$results["type_wm"];$op_area=$results["op_area"];$hav_u=$results["hav_u"];$stamp_details=$results["stamp_details"];$state=$results["state"];$lic_fee=$results["lic_fee"];$lic_fee_words=$results["lic_fee_words"];
			$bank_sub_date=$results["bank_sub_date"];
			if($bank_sub_date!="" && $bank_sub_date!="0000-00-00"){
				$bank_sub_date = date('d-m-Y',strtotime($bank_sub_date));
			}else{
				$bank_sub_date="";
			}
