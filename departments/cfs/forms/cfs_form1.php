...
if($q->num_rows<1){	 
	...
	if($p->num_rows>0){
		...
		// Tab 3 //
		$electricity=$results['electricity'];$is_unit=$results['is_unit'];$is_unit_details=$results['is_unit_details'];$rupees=$results['rupees'];$draft=$results['draft'];
		...
		$dairy_lean="";$dairy_flush="";
		// Tab 3 //
		$electricity="";$is_unit="";$is_unit_details="";$rupees="";$draft="";
		$factory_name="";$factory_sn1="";$factory_sn2="";$factory_vill="";$factory_dist="";$factory_pin="";$factory_mobile="";
	}
}else{
	...
	if(!empty($results["in_charge"])){
		$in_charge=json_decode($results["in_charge"]);
        if(isset($in_charge->name)) $in_charge_name=$in_charge->name; else $in_charge_name="";
        if(isset($in_charge->qual)) $in_charge_qual=$in_charge->qual; else $in_charge_qual="";
        if(isset($in_charge->address)) $in_charge_address=$in_charge->address; else $in_charge_address="";
        if(isset($in_charge->mobile)) $in_charge_mobile=$in_charge->mobile; else $in_charge_mobile="";
        if(isset($in_charge->tel)) $in_charge_tel=$in_charge->tel; else $in_charge_tel="";
        if(isset($in_charge->email)) $in_charge_email=$in_charge->email; else $in_charge_email="";
        if(isset($in_charge->card)) $in_charge_card=$in_charge->card; else $in_charge_card="";
        if(isset($in_charge->expiry)) $in_charge_expiry=$in_charge->expiry; else $in_charge_expiry="";
									...
									<form name="myform1" class="myform1 submit1" method="post"  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
										<table class="table table-responsive table-bordered">
											...
											<tr>
												...
												<td colspan="3">
													<label class="radio-inline"><input type="radio" name="is_license" value="Y" <?php if(isset($is_license) && $is_license=='Y') echo 'checked'; ?> /> Yes </label>
													<label class="radio-inline"><input type="radio" name="is_license" value="N" <?php if(isset($is_license) && ($is_license=='N' || $is_license=='')) echo 'checked'; ?>/> No </label>
												</td>												
											</tr>
											<tr>
