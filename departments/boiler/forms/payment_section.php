...
include "save_boiler_form7.php";
include "save_form8.php";
include "save_form9.php";
include "save_boiler_form10.php";

if(isset($_GET["token"])){
	$token=$_GET["token"];
	$check=$formFunctions->is_already_registered('boiler',$token);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=boiler';
			</script>";	
	}else if($check==0){
		echo "<script>
		...
		echo "<script>
				window.location.href = '".$server_url."departments/boiler/forms/boiler_form".$token.".php';
		</script>";	
	
	}else{
		$submit_payment=$token;
		$reg_fees=$boiler->query("select reg_fees from boiler_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->reg_fees;
	}
}else{
	echo "<script>
			alert('Something went wrong!!!');
											...
											<div class="row">
												<div class="col-md-4 col-md-offset-4 box box-shadow" style="width:30%;padding-bottom:30px">
													<p><b><u>Challan Head</u></b><br/>
														0230- Labour and Employment ,<br/>103- Fee for inspection of Steam Boilers<br/><br/>
													</p>
													<div class="row">
