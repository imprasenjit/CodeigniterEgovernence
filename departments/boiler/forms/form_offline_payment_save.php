...
if(isset($_POST["submit_fees"])){
	...
	if($_POST["payment_mode"]==1){
		...
		if(empty($_POST["offline_challan"]) || $_POST["offline_challan"]=='2' || $_POST["offline_challan"]=='3'){
			...
			$save_query=$boiler->query("update boiler_form".$token."_certificates set offline_fees_challan='$offline_fees_challan',is_paid='Y' where form_id='$form_id'") or die($boiler->error);
			if($save_query){					
