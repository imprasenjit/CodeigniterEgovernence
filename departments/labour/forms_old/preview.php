...
$css="";	
$applicant_id=$sid;
include "save_reg_form.php";
include "save_license_form.php";
include "save_renewal_form.php";	
if(isset($_GET["token"])){
	$token=$_GET["token"];
	$check=$formFunctions->is_already_registered('labour',$token);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=labour';
			</script>";	
	}else if($check==0){
		echo "<script>
	...
	if($token>0 && ($token<6 || $token==13)){
		include "lc_reg_form".$token."_print.php";
	}else if($token>5 && $token<9){
		include "lc_license_form".$token."_print.php";
	}else if($token>8 && $token<13){
		include "lc_renewal_form".$token."_print.php";
	}else{}
}else{
	echo "<script>
			alert('Something went wrong!!!');
  ...
  <div class="container-fluid avoid_me">
	...
	<div class="navbar-header col-md-12 text-center">	
	  ...
	  <?php if(isset($show_edit)){ ?>
			<?php if($token>0 && ($token<6 || $token==13)){ ?>
				<a type="button" href="lc_reg_form<?php echo $token; ?>.php" class="btn btn-app bg-light-blue"><i class="fa fa-edit"></i>Edit</a>
			<?php }else if($token>5 && $token<9){ ?>
				<a type="button" href="lc_license_form<?php echo $token; ?>.php" class="btn btn-app bg-light-blue"><i class="fa fa-edit"></i>Edit</a>
			<?php }else{ ?>
				<a type="button" href="lc_renewal_form<?php echo $token; ?>.php" class="btn btn-app bg-light-blue"><i class="fa fa-edit"></i>Edit</a>
			<?php } ?>
