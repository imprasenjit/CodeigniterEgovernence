...
if(!function_exists('getZone')){
	 function getZone($gmc_zone){
			if($gmc_zone=="1" || $gmc_zone=="2" || $gmc_zone=="3" || $gmc_zone=="4" || $gmc_zone=="5" || $gmc_zone=="7" || $gmc_zone=="8"){
				$gmc_zone_name="West Zone";
			}else if($gmc_zone=="10" || $gmc_zone=="11" || $gmc_zone=="12" || $gmc_zone=="13"){
				$gmc_zone_name="South Zone";
			}else if($gmc_zone=="19" || $gmc_zone=="20" || $gmc_zone=="21" || $gmc_zone=="22" || $gmc_zone=="23" || $gmc_zone=="24"){
				$gmc_zone_name="East Zone";
			}else if($gmc_zone=="9" || $gmc_zone=="14" || $gmc_zone=="15" || $gmc_zone=="18"){
				$gmc_zone_name="Central Zone";
			}else if($gmc_zone=="25" || $gmc_zone=="26" || $gmc_zone=="28" || $gmc_zone=="29" || $gmc_zone=="30" || $gmc_zone=="31"){
				$gmc_zone_name="Dispur Zone";
			}else{
				$gmc_zone_name="Lokhora Zone";
			}
			return $gmc_zone_name;
	}   
}
...
// End of online_payment_details()		
    ...
    public function create_uain($form_id, $dept, $form) {
        global $unit_id;
        $sqla = $this->executeQuery("dicc", "select c.dist_code from unit_master_record as a LEFT JOIN address as b ON a.address = b.id LEFT JOIN districts as c ON c.dist_id = b.dist where a.unit_id='$unit_id'");
        $rowa = $sqla->fetch_object();
        $dist_code = $rowa->dist_code;
	...
	public function getOffices($dept){		
		$query="SELECT * FROM offices";		
		$rows=$this->executeQuery($dept,$query);			
		$masterArray=array();
		while($result=$rows->fetch_assoc()){
		    $tempArray=array('id'=>$result['id'],'office_name'=>$result['office_name'],'street1'=>$result['street1'],'street2'=>$result['street2'],'city'=>$result['city'],'district'=>$result['district'],'pin'=>$result['pin'],'email'=>$result['email'],'mobile_no'=>$result['mobile_no'],'jurisdiction'=>$result['jurisdiction']);     
    ...
    public function insert_applications($uain) {
        ...
        if ($dept == "fire") {
        ...
        } else if ($dept == "dic" && ($form == 10 || $form == 11)) {
            $fetch_data = "officer_id,courier_details,save_mode,received_date";
        } else if ($dept == "doa") {
            $fetch_data = "office_id,officer_id,courier_details,save_mode,received_date";
        } else {
            ...
            if ($office_details->num_rows > 0) {
				...
				echo "Please check the jurisdiction in offices table";
				die();
                return false;
                exit();
            }
                ...
                if ($check_query->num_rows == 0) {
                    switch ($dept) {
                        case "fire":
                            ...
                            $process_ids = $this->executeQuery("fire", "select user_id,office_id from users where fire_station='$nearest_station'");
										
							if($process_ids->num_rows>0){
								$process_ids_row = $process_ids->fetch_assoc();
								$office_id 	= 	$process_ids_row["office_id"];
								
								$process_user_id 	= 	$process_ids_row["user_id"];
							}else{
								return false;
								exit();
							}				
                        ...
                        case "dic":
                            ...
                            $office_id = $check_query_row->office_id;
