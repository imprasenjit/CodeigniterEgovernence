...
if(isset($_GET["token"]) && !empty($_GET["token"]) && is_numeric($_GET["token"])){
	...
	$check=$formFunctions->executeQuery($dept,"select process_type from ".$table_name." a, ".$table_name."_process b, ".$table_name."_certificates c where a.user_id='$swr_id' and b.form_id=a.form_id and b.process_type='A' and c.is_paid!='Y'") or die($forest->error);
	if($check->num_rows>0){
		...
		$fees=$formFunctions->executeQuery($dept,"select total_fees from ".$table_name."_certificates where form_id='$form_id' and is_paid!='Y'")->fetch_object()->total_fees;
		$prev="../index.php";
	}
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$formFunctions->executeQuery($dept,"select form_name from forest_form_names where form_no='".$token."'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
