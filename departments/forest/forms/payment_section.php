...
if(isset($_GET["token"])){
	$token=$_GET["token"];
	$check=$formFunctions->is_already_registered('forest',$token);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=forest';
			</script>";	
	}else if($check==0){
		echo "<script>
		...
		echo "<script>
				window.location.href = '".$server_url."departments/forest/forms/forest_form".$token.".php';
		</script>";	
	}else{
		$submit_payment=$token;
		$reg_fees=100;
		//$license_fee=$forest->query("select license_fee from forest_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->license_fee;
	}
}else{
	die();
	echo "<script>
			alert('Something went wrong!!!');
			window.location.href = '".$server_url."departments/forest/';
	</script>";
}
?>
<!DOCTYPE html>
