...
$table_name=getTableName($dept,$form);
	...
	if($q->num_rows>0){
		...
		if($pcb_guidelines=="Y") $pcb_guidelines="YES";
		else $pcb_guidelines="NO";
...
if(!empty($results["uain"])){
    ...
    <table class="table table-bordered table-responsive">  
		...
		<tr>
			<td valign="top" style="width:60%">1. (a) Name and address of the unit and location of facility :</td>
			<td>
				<table class="table table-bordered table-responsive"> 
				<tr>
						<td>Name</td>
		...
		<tr>
			...
			<td>
				...
				'.strtoupper($auth_req_str).'<br/>
				'.strtoupper($auth_req_transport).'<br/>
				'.strtoupper($auth_req_recept).'<br/>
				'.strtoupper($auth_req_reuse).'<br/>
				'.strtoupper($auth_req_recycle).'<br/>
				'.strtoupper($auth_req_rec).'<br/>
				'.strtoupper($auth_req_pre).'<br/>
				'.strtoupper($auth_req_co).'<br/>
				'.strtoupper($auth_req_uti).'<br/>
				'.strtoupper($auth_req_treat).'<br/>
				'.strtoupper($auth_req_disp).'<br/>
				'.strtoupper($auth_req_inci).'<br/>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td valign="top" colspan="2"><p>2.(a) Nature and quantity of waste handled per annum (in metric tonne or kilo litre):</p></td>
		</tr>
		<tr>
			<td colspan="2">
				...
				<tr>
					...
					$part1=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t1 WHERE form_id='$form_id'");
						while($row_1=$part1->fetch_array()){
							if($row_1["unit"]=="MT"){
								$unit_nq="in metric tonnes / month";
							}elseif($row_1["unit"]=="K"){
								$unit_nq="in kl / month";
				...
				<tr>
					...
					$part2=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t2 WHERE form_id='$form_id'");
						while($row_1=$part2->fetch_array()){
							if($row_1["unit"]=="MT"){
								$unit_nqw="in metric tonnes / month";
							}elseif($row_1["unit"]=="K"){
								$unit_nqw="in kl / month";
							}else if($row_1["unit"]=="KG"){
				                 $unit_nqw="in kg / month";
							}else{
								$unit_nqw="";
							}
						$printContents=$printContents.'
						<tr>
							<td>'.strtoupper($row_1["sl_no"]).'</td>
							<td>'.strtoupper($row_1["particulars"]).'</td>
							<td>'.strtoupper($row_1["nature"]).'</td>
							<td>'.strtoupper($row_1["qty"]).'</td>
							<td>'.strtoupper($unit_nqw).'</td>
						</tr>';
						}$printContents=$printContents.'
				</table>
			</td>
				...
				<tr>
					...
					$part3=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t3 WHERE form_id='$form_id'");
						while($row_1=$part3->fetch_array()){
							if($row_1["product_type"]=="P"){
								$product_type_pb="Product";
							}elseif($row_1["product_type"]=="B"){
								$product_type_pb="By-product";
							}
							if($row_1["unit"]=="T"){
								$unit_pb="in tonnes / month";
							}elseif($row_1["unit"]=="K"){
								$unit_pb="in kl / month";
							}elseif($row_1["unit"]=="N"){
								$unit_pb="in numbers / month";
							}else if($row_1["unit"]=="KG"){
								$unit_pb="in kg / month";
							}else{
								$unit_pb="";
						...
						<tr>
							...
							<td>'.strtoupper($product_type_pb).'</td>
							<td>'.strtoupper($row_1["qty"]).'</td>
							<td>'.strtoupper($unit_pb).'</td>
						</tr>';
						}$printContents=$printContents.'
				</table>
			</td>
				...
				<tr>
					...
					$part4=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t4 WHERE form_id='$form_id'");
						while($row_1=$part4->fetch_array()){
							if($row_1["unit"]=="T"){
								$unit_ww="in tonnes / month";
							}elseif($row_1["unit"]=="K"){
								$unit_ww="in kl / month";
							}elseif($row_1["unit"]=="N"){
								$unit_ww="in numbers / month";
							}else if($row_1["unit"]=="KG"){
							   $unit_ww="in kg / month";
							}else{
							   $unit_ww="";
						...
						<tr>
							<td>'.strtoupper($row_1["slno"]).'</td>
							<td>'.strtoupper($row_1["particulars"]).'</td>
							<td>'.strtoupper($row_1["characteristics"]).'</td>
							<td>'.strtoupper($row_1["qty"]).'</td>
							<td>'.strtoupper($unit_ww).'</td>
						</tr>';
						}$printContents=$printContents.'
				</table>
			</td>
		...
		<tr>
			<td valign="top">i. Capacity and mode of secured storage within the plant:</td>
				...
				<tr>
					...
					$part5=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t5 WHERE form_id='$form_id'");
						while($row_1=$part5->fetch_array()){
							if($row_1["nature_type"]=="HW"){
								$nature_type_ho="Hazardous Waste";
							}elseif($row_1["nature_type"]=="OW"){
								$nature_type_ho="Other Waste";
							}
							if($row_1["unit"]=="T"){
								$unit_ho="in tonnes / month";
							}elseif($row_1["unit"]=="K"){
								$unit_ho="in kl / month";
							}elseif($row_1["unit"]=="N"){
								$unit_ho="in numbers / month";
							}else if($row_1["unit"]=="KG"){
							   $unit_ho="in kg / month";
							}else{
							   $unit_ho="";
						...
						<tr>
							<td>'.strtoupper($row_1["slno"]).'</td>
							<td>'.strtoupper($row_1["particulars"]).'</td>
							<td>'.strtoupper($nature_type_ho).'</td>
							<td>'.strtoupper($row_1["qty"]).'</td>
							<td>'.strtoupper($unit_ho).'</td>
						</tr>';
						}$printContents=$printContents.'
				</table>
			</td>
		...
		<tr>
			<td valign="top">(i). Location of site with layout map:</td>
			<td>Document is attached</td>
		</tr>
		<tr>
				...
				<tr>
					...
					$part6=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t6 WHERE form_id='$form_id'");
						while($row_1=$part6->fetch_array()){
							if($row_1["unit"]=="T"){
								$unit_ss="in tonnes / month";
							}elseif($row_1["unit"]=="K"){
								$unit_ss="in kl / month";
							}elseif($row_1["unit"]=="N"){
								$unit_ss="in numbers / month";
							}else if($row_1["unit"]=="KG"){
							   $unit_ss="in kg / month";
							}else{
							   $unit_ss="";
			...
			<td colspan="2">
				...
				<tr>
					...
					$part7=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t7 WHERE form_id='$form_id'");
						while($row_1=$part7->fetch_array()){
							if($row_1["source"]=="D"){
								$source_nq="Domestic";
							}elseif($row_1["source"]=="I"){
								$source_nq="Imported";
							}elseif($row_1["source"]=="B"){
								$source_nq="Both";
							}else{
								$source_nq="";
							}
			...
			<td colspan="2">
				...
				<tr>
					...
					$part8=$formFunctions->executeQuery($dept,"SELECT * FROM ".$table_name."_t8 WHERE form_id='$form_id'");
						while($row_1=$part8->fetch_array()){
							if($row_1["product_type"]=="P"){
								$product_type_pd="Product";
							}elseif($row_1["product_type"]=="B"){
								$product_type_pd="By-product";
							}else{
								$product_type_pd="";
							}
							if($row_1["unit"]=="T"){
								$unit_pd="in tonnes / month";
							}elseif($row_1["unit"]=="K"){
								$unit_pd="in kl / month";
							}elseif($row_1["unit"]=="N"){
								$unit_pd="in numbers / month";
							}elseif($row_1["unit"]=="KG"){
							   $unit_pd="in kg / month";
							}else{
							   $unit_pd="";
							}
						...
						<tr>
							<td>'.strtoupper($row_1["slno"]).'</td>
							<td>'.strtoupper($row_1["name"]).'</td>
							<td>'.strtoupper($product_type_pd).'</td>
							<td>'.strtoupper($row_1["qty"]).'</td>
							<td>'.strtoupper($unit_pd).'</td>
						</tr>';
						}$printContents=$printContents.'
				</table>
			</td>
