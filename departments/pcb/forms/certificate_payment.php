...
$uian="";
$is_paid="N";
$query_fees="Other Fees";
if(isset($_GET["token"]) && !empty($_GET["token"]) && is_numeric($_GET["token"])){
	$token=$_GET["token"];
	$table_name=$formFunctions->getTableName("pcb",$token);
	$formName=$cms->query("select form_name from pcb_form_names where form_no='".$token."'")->fetch_object()->form_name;
	$sql=$pcb->query("select form_id,uain from ".$table_name." where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uian=$row['uain'];
		#certificate check
		$sqlC=$pcb->query("select total_fees,is_paid from ".$table_name."_certificates where form_id='$form_id'") or die("Error : ".$pcb->error);
...
$returnUrl=$server_url."pcb/certificate_payment.php?token=".$token;
...
$newData="DOIACGOA|".$sentCustomerID."|NA|".$certificate_fees."|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|PCB001|".$txDateTime."|".$formName."|".$query_fees."|NA|".$returnUrl;
...
<?php if($formName=="CTE" || $formName=="CTO"){ ?>
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from pcb_form_names where form_no='1'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
							<center><div style="width:300px" class="text-center alert alert-warning" role="alert"><h4 style="color: #230C0F"><strong>Certificate Fee Payment</strong></h4></div></center>
