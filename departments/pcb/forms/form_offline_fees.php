...
if(isset($_GET["token"]) && !empty($_GET["token"]) && is_numeric($_GET["token"])){
	...
	$check=$pcb->query("select process_type from ".$table_name." a, ".$table_name."_process b, ".$table_name."_certificates c where a.user_id='$swr_id' and b.form_id=a.form_id and b.process_type='A' and c.is_paid!='Y'") or die($pcb->error);
	if($check->num_rows>0){
		$form_id=$pcb->query("select form_id from ".$table_name." where user_id='$swr_id' and active='1'")->fetch_object()->form_id;
		$fees=$pcb->query("select total_fees from ".$table_name."_certificates where form_id='$form_id' and is_paid!='Y'")->fetch_object()->total_fees;
		$prev="../index.php";
	}
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from pcb_form_names where form_no='".$token."'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
