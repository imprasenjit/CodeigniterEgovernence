...
if(isset($_GET["token"]) && is_numeric($_GET["token"]) && $_GET["token"]>0 && $_GET["token"]<57 ){
	$token=$_GET["token"];	
	...
	if($token<3){
		include "form".$token."_print.php";
	}else{
		include "pcb_form".$token."_print.php";
	}
}else{
	echo "<script>
			alert('Something went wrong!!!');
  ...
  <div class="container-fluid avoid_me">
	...
	<div class="navbar-header col-md-12 text-center">	
	  ...
	  <?php if(isset($show_edit)){ ?>
			<?php if($token>0 && $token<3){ ?>
				<a type="button" href="form<?php echo $token; ?>.php" class="btn btn-app bg-light-blue"><i class="fa fa-edit"></i>Edit</a>
			<?php }else {?>
				<a type="button" href="pcb_form<?php echo $token; ?>.php" class="btn btn-app bg-light-blue"><i class="fa fa-edit"></i>Edit</a>
			<?php } ?>
