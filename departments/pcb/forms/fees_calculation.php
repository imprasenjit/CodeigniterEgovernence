...
function fees_calculation($uain){
	...
	if($form==1 || $form==3 || $form==49){
		$fees_table="cte_fees_schedule";
		$results=$formFunctions->executeQuery("pcb","select form_id,investment_cost from ".$table_name." where uain='$uain'") or  die($pcb->error);
	} else {
		$fees_table="cto_fees_schedule";
		$results=$formFunctions->executeQuery("pcb","select form_id,investment_cost,application_for from ".$table_name." where uain='$uain'") or  die($pcb->error);
	}
	...
	if($results->num_rows>0){
		...
		if(!empty($row["investment_cost"])){
			...
			$fees_results=$formFunctions->executeQuery("pcb","select * from ".$fees_table." where min_invest<$investment_cost and max_invest>=$investment_cost") or die($pcb->error);
...
function dg_set_fees_calculation($uain){
	...
	if($form!=49){
		...
		if($results->num_rows>0){
			...
			$form_id=$row->form_id;		
			$results=$formFunctions->executeQuery("pcb","select * from ".$table_name."_dgsets where form_id='$form_id'") or  die($pcb->error);
			if($results->num_rows>0){
				while($rows=$results->fetch_object()){
					$dg_invest=$rows->dg_invest;
					$fees_results=$formFunctions->executeQuery("pcb","select * from ".$fees_table." where min_invest<$dg_invest and max_invest>=$dg_invest") or die($pcb->error);
					if($fees_results->num_rows>0){
						while($fees_details=$fees_results->fetch_object()){
							//$dgsets_reg_fees=$fees_details->fee;
							$dgsets_reg_fees=$dgsets_reg_fees+$fees_details->fee;
						}								
					}
				}
