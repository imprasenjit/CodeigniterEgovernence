...
if(isset($_POST["save1a"])){		
	...
	$sql=$formFunctions->executeQuery($dept,"select form_id from pcb_form1 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	
	if($sql->num_rows<1){
		if($file=="SC"){
			$query=$formFunctions->executeQueryInsertID($dept,"insert into pcb_form1(user_id,sub_date,courier_details,revenue_survey_no,plan_details,lb_name,lb_auth_name,md_name,md_address,is_registered,reg_no,reg_date) values ('$swr_id','$today','1', '$revenue_survey_no', '$plan_details', '$lb_name', '$lb_auth_name', '$md_name', '$md_address', '$is_registered', '$reg_no', '$reg_date')");
		}else{
			$query=$formFunctions->executeQuery($dept,"insert into pcb_form1(user_id,sub_date,revenue_survey_no,plan_details,lb_name,lb_auth_name,md_name,md_address,is_registered,reg_no,reg_date) values ('$swr_id','$today', '$revenue_survey_no', '$plan_details', '$lb_name', '$lb_auth_name', '$md_name', '$md_address', '$is_registered', '$reg_no', '$reg_date')");
		}
		
		$form_id=$query;
		$query2=$formFunctions->executeQuery($dept,"insert into pcb_form1_part1(form_id) values ('$form_id')");
		$query3=$formFunctions->executeQuery($dept,"insert into pcb_form1_part2(form_id) values ('$form_id')");
		$query4=$formFunctions->executeQuery($dept,"insert into pcb_form1_upload(form_id) values ('$form_id')") ;
		...
		if($file=="SC"){
			$query=$formFunctions->executeQuery($dept,"update pcb_form1 set sub_date='$today',courier_details='1', revenue_survey_no='$revenue_survey_no', plan_details='$plan_details', lb_name='$lb_name', lb_auth_name='$lb_auth_name',md_name='$md_name', md_address='$md_address', is_registered='$is_registered', reg_no='$reg_no', reg_date='$reg_date' where form_id='$form_id'");
		}else{
			$query=$formFunctions->executeQuery($dept,"update pcb_form1 set sub_date='$today', revenue_survey_no='$revenue_survey_no', plan_details='$plan_details', lb_name='$lb_name', lb_auth_name='$lb_auth_name',md_name='$md_name', md_address='$md_address', is_registered='$is_registered', reg_no='$reg_no', reg_date='$reg_date' where form_id='$form_id'") ;
		}
...
if(isset($_POST["save1b"])){
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
		...
		if($input_size1!=0){					
			$k=$formFunctions->executeQuery($dept,"delete from pcb_form1_products where form_id='$form_id'");
			...
			$part1=$formFunctions->executeQuery($dept,"INSERT INTO pcb_form1_products(form_id,slno,name,product_type,qty,unit) VALUES ('$form_id','$i','$valb','$valc','$vald','$vale')");
			}
		}
		if($input_size2!=0){					
			$k=$formFunctions->executeQuery($dept,"delete from pcb_form1_materials where form_id='$form_id'");
			...
			$part2=$formFunctions->executeQuery($dept,"INSERT INTO pcb_form1_materials(form_id,slno,name,material_type,qty,unit) VALUES ('$form_id','$i','$valb','$valc','$vald','$vale')");
			}
		}
...
if(isset($_POST["save1c"])) {
	...
	if($sql->num_rows<1){
		...
		$form_id=$row["form_id"];
		if($file=="SC"){
			$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1 SET courier_details='1' WHERE form_id='$form_id'");
		}
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1_part1 SET water_source='$water_source',budget_calc='$budget_calc',sewage_treatment='$sewage_treatment',wc_values='$wc_values',ww_qty='$ww_qty' WHERE form_id='$form_id'");
...
if(isset($_POST["save1d"])) {
	...
	if($sql->num_rows<1){				
		...
		if($laboratory_report=="SC"){
			$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1 SET courier_details='1' WHERE form_id='$form_id'");
		}
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1_part1 SET trade_treatment='$trade_treatment',is_mixed='$is_mixed',yes_detail='$yes_detail',sump_capacity='$sump_capacity',effluents_quality='$effluents_quality',laboratory_report='$laboratory_report', disposal_mode='$disposal_mode' WHERE form_id='$form_id'");
...
if(isset($_POST["save1e"])) {
	...
	if($sql->num_rows<1){				
		...
		$row=$sql->fetch_array();
		$form_id=$row["form_id"];
		if($spec_residue_report=="SC" || $gas_quality=="SC"){
			$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1 SET courier_details='1' WHERE form_id='$form_id'");
		}
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1_part1 SET spec_residue_report='$spec_residue_report',is_odoriferous='$is_odoriferous',is_adq_facility='$is_adq_facility', fuel_consumption='$fuel_consumption', stack_details='$stack_details', gas_quality='$gas_quality' WHERE form_id='$form_id'") ;
...
if(isset($_POST["save1f"])) {
	...
	if($sql->num_rows<1){				
		...
		$row=$sql->fetch_array();
		$form_id=$row["form_id"];
		if($haz_character=="SC" || $haz_qty_dispose_file=="SC"){
			$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1 SET courier_details='1' WHERE form_id='$form_id'");
		}
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form1_part2 SET is_hazardous='$is_hazardous',haz_cat_no='$haz_cat_no', haz_qty='$haz_qty', haz_character='$haz_character', storage_mode='$storage_mode', haz_pres_treatment='$haz_pres_treatment',auth_req='$auth_req',haz_qty_dispose='$haz_qty_dispose' WHERE form_id='$form_id'") ;
...
if(isset($_POST["save1h"])){	
	if((isset($_POST["mfile16"]) && empty($_POST["mfile16"])) || (isset($_POST["mfile17"]) && empty($_POST["mfile17"])) || (isset($_POST["mfile18"]) && empty($_POST["mfile18"])) || (isset($_POST["mfile19"]) && empty($_POST["mfile19"])) || (isset($_POST["mfile20"]) && empty($_POST["mfile20"])) || (isset($_POST["mfile21"]) && empty($_POST["mfile21"])) || (isset($_POST["mfile22"]) && empty($_POST["mfile22"])) || (isset($_POST["mfile23"]) && empty($_POST["mfile23"])) || (isset($_POST["mfile24"]) && empty($_POST["mfile24"])) || (isset($_POST["mfile16"]) && $_POST["mfile16"]=='2') || (isset($_POST["mfile17"]) && $_POST["mfile17"]=='2') || (isset($_POST["mfile18"]) && $_POST["mfile18"]=='2') || (isset($_POST["mfile19"]) && $_POST["mfile19"]=='2') || (isset($_POST["mfile20"]) && $_POST["mfile20"]=='2') || (isset($_POST["mfile21"]) && $_POST["mfile21"]=='2') || (isset($_POST["mfile22"]) && $_POST["mfile22"]=='2') || (isset($_POST["mfile23"]) && $_POST["mfile23"]=='2') || (isset($_POST["mfile24"]) && $_POST["mfile24"]=='2') || (isset($_POST["mfile16"]) && $_POST["mfile16"]=='3') || (isset($_POST["mfile17"]) && $_POST["mfile17"]=='3') || (isset($_POST["mfile18"]) && $_POST["mfile18"]=='3') || (isset($_POST["mfile19"]) && $_POST["mfile19"]=='3') || (isset($_POST["mfile20"]) && $_POST["mfile20"]=='3') || (isset($_POST["mfile21"]) && $_POST["mfile21"]=='3') || (isset($_POST["mfile22"]) && $_POST["mfile22"]=='3') || (isset($_POST["mfile23"]) && $_POST["mfile23"]=='3') || (isset($_POST["mfile24"]) && $_POST["mfile24"]=='3')){
		...
		if($sql->num_rows<1){				
			...
			$row=$sql->fetch_array();
			$form_id=$row["form_id"];
			$savequery=$formFunctions->executeQuery($dept,"update pcb_form1_upload set file1='$file1',file2='$file2',file3='$file3',file4='$file4',file5='$file5',file6='$file6',file7='$file7',file8='$file8',file9='$file9' where form_id='$form_id'") ;
		}		
		if($savequery){
			...
			if($file1="SC" || $file2="SC" ||  $file3="SC" ||  $file4="SC" ||  $file5="SC" ||  $file6="SC" ||  $file7="SC" ||  $file8="SC" ||  $file9="SC"){
				$save_query=$formFunctions->executeQuery($dept,"update pcb_form1 set courier_details='1', sub_date='$today' where form_id='$form_id'");
			}else{
				$save_query=$formFunctions->executeQuery($dept,"update pcb_form1 set sub_date='$today' where form_id='$form_id'");
			}
			if($save_query){
				echo "<script>
					alert('Successfully Saved....');
...
if(isset($_POST["proceed1"])){		
	$sql=$formFunctions->executeQuery($dept,"select form_id,save_mode,courier_details from pcb_form1 where user_id='$swr_id' and active='1'");		
	if($sql->num_rows<1){				
		echo "<script>
			alert('Please fill the first part of the form.');
			window.location.href = 'form1.php';
		...
		if($save_mode=="D" && $courier_details==1){
			$save_query=$formFunctions->executeQuery($dept,"update pcb_form1 set sub_date='$today',uain='$uain',save_mode='F' where form_id='$form_id'");
			if($save_query){
				echo "<script>
					alert('Successfully Saved.');
					window.location.href = '../../requires/courier_details.php?dept=pcb&form=1';
				</script>";
			}else{
				echo "<script>
					alert('Something went wrong !!!');
			...
			$save_query=$formFunctions->executeQuery($dept,"update pcb_form1 set sub_date='$today',uain='$uain',save_mode='P' where form_id='$form_id'");
			if($save_query){
				echo "<script>
					alert('Successfully Saved.');
	...
	if($_POST["payment_mode"]==1){
		...
		if(empty($_POST["offline_challan"]) || $_POST["offline_challan"]=='2' || $_POST["offline_challan"]=='3'){
			...
			if($sql->num_rows>0){
				...
				$save_query=$formFunctions->executeQuery($dept,"update pcb_form1 set offline_challan='$offline_challan',payment_mode='$payment_mode',save_mode='C',sub_date='$today' where form_id='$form_id'");
				
				if($save_query){
					$uain=$formFunctions->executeQuery($dept,"select uain from pcb_form1 where form_id='$form_id'")->fetch_object()->uain;
...
if(isset($_POST["save2a"])){		
	...
	$sql=$formFunctions->executeQuery($dept,"select form_id from pcb_form2 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	
	if($sql->num_rows<1){
		if($file=="SC"){
			$query=$formFunctions->executeQueryInsertID($dept,"insert into pcb_form2(user_id,sub_date,courier_details,revenue_survey_no,plan_details,lb_name,lb_auth_name,md_name,md_address,is_registered,reg_no,reg_date) values ('$swr_id','$today', '1','$revenue_survey_no', '$plan_details', '$lb_name', '$lb_auth_name', '$md_name', '$md_address', '$is_registered', '$reg_no', '$reg_date')");  
		}else{
			$query=$formFunctions->executeQuery($dept,"insert into pcb_form2(user_id,sub_date,courier_details,revenue_survey_no,plan_details,lb_name,lb_auth_name,md_name,md_address,is_registered,reg_no,reg_date) values ('$swr_id','$today','1','$revenue_survey_no', '$plan_details', '$lb_name', '$lb_auth_name', '$md_name', '$md_address', '$is_registered', '$reg_no', '$reg_date')");
		}
		if($query){
			$form_id=$query;
			$query2=$formFunctions->executeQuery($dept,"insert into pcb_form2_part1(form_id) values ('$form_id')");
			$query3=$formFunctions->executeQuery($dept,"insert into pcb_form2_part2(form_id) values ('$form_id')");
			$query4=$formFunctions->executeQuery($dept,"insert into pcb_form2_upload(form_id) values ('$form_id')");
			if($query2==false || $query3==false || $query4==false){
				echo "<script>
					alert('Something went wrong !!!');
		...
		$form_id=$row["form_id"];
		if($file=="SC"){
			$query=$formFunctions->executeQuery($dept,"update pcb_form2 set sub_date='$today',courier_details='1', revenue_survey_no='$revenue_survey_no', plan_details='$plan_details', lb_name='$lb_name', lb_auth_name='$lb_auth_name',md_name='$md_name', md_address='$md_address', is_registered='$is_registered', reg_no='$reg_no', reg_date='$reg_date' where form_id='$form_id'");
		}else{
			$query=$formFunctions->executeQuery($dept,"update pcb_form2 set sub_date='$today', revenue_survey_no='$revenue_survey_no', plan_details='$plan_details', lb_name='$lb_name', lb_auth_name='$lb_auth_name',md_name='$md_name', md_address='$md_address', is_registered='$is_registered', reg_no='$reg_no', reg_date='$reg_date' where form_id='$form_id'") ;
		}
		if($query){
			$formFunctions->file_update($file);
...
if(isset($_POST["save2b"])){
	...
	if($sql->num_rows<1){   ////////////table is empty//////////////				
		...
		$form_id=$row["form_id"];
		if($file1=="SC" || $file2=="SC" || $file3=="SC"){
			$save_query="UPDATE pcb_form2 SET courier_details='1',investment_certificate='$investment_certificate',investment_cost='$investment_cost',site_distance='$site_distance',wb_name='$wb_name',loc_feedback='$loc_feedback',is_provided='$is_provided',is_use='$is_use',is_not_details='$is_not_details',staff_nos='$staff_nos',is_res_colony='$is_res_colony',process_desc='$process_desc',total_plot_area='$total_plot_area',commission_my='$commission_my',colony_details='$colony_details' WHERE form_id='$form_id'";
			$query = $formFunctions->executeQuery($dept,$save_query);
		}else{
		...
		if($input_size1!=0){					
			...
			$part1=$formFunctions->executeQuery($dept,"INSERT INTO pcb_form2_products(form_id,slno,name,product_type,qty,unit) VALUES ('$form_id','$i','$valb','$valc','$vald','$vale')");
			}
		}
		if($input_size2!=0){					
			...
			$part2=$formFunctions->executeQuery($dept,"INSERT INTO pcb_form2_materials(form_id,slno,name,material_type,qty,unit) VALUES ('$form_id','$i','$valb','$valc','$vald','$vale')");
			}
		}		
	}	
	if($query){		
...
if(isset($_POST["save2c"])) {
	...
	if($sql->num_rows<1){
		...
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form2_part1 SET water_source='$water_source',budget_calc='$budget_calc',sewage_treatment='$sewage_treatment',wc_values='$wc_values',ww_qty='$ww_qty' WHERE form_id='$form_id'");
...
if(isset($_POST["save2d"])) {
	...
	if($sql->num_rows<1){				
		...
		$form_id=$row["form_id"];
		if($laboratory_report=="SC"){
			$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form2 SET courier_details='1' WHERE form_id='$form_id'");
		}
		
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form2_part1 SET trade_treatment='$trade_treatment',is_mixed='$is_mixed',yes_detail='$yes_detail',sump_capacity='$sump_capacity',effluents_quality='$effluents_quality',laboratory_report='$laboratory_report', disposal_mode='$disposal_mode' WHERE form_id='$form_id'");
...
if(isset($_POST["save2e"])) {
	...
	if($sql->num_rows<1){				
		...
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form2_part1 SET spec_residue_report='$spec_residue_report',is_odoriferous='$is_odoriferous',is_adq_facility='$is_adq_facility', fuel_consumption='$fuel_consumption', stack_details='$stack_details', gas_quality='$gas_quality' WHERE form_id='$form_id'");
...
if(isset($_POST["save2f"])) {
	...
	if($sql->num_rows<1){				
		...
		if($haz_character=="SC" || $haz_qty_dispose_file=="SC"){
		    ...
		    $savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form2_part2 SET is_hazardous='$is_hazardous',haz_cat_no='$haz_cat_no', haz_qty='$haz_qty', haz_character='$haz_character', storage_mode='$storage_mode', haz_pres_treatment='$haz_pres_treatment',auth_req='$auth_req',haz_qty_dispose='$haz_qty_dispose' WHERE form_id='$form_id'");
...
if(isset($_POST["save2g"])) {
	...
	if($sql->num_rows<1){				
		...
		$savequery=$formFunctions->executeQuery($dept,"UPDATE pcb_form2_part2 SET is_sys_upg='$is_sys_upg',om_expens='$om_expens', to_which='$to_which', dgset_items='$dgset_items', nonhaz_details='$nonhaz_details', haz_chemicals='$haz_chemicals',tree_plant='$tree_plant',scheme_info='$scheme_info', other_info='$other_info',sys_upg_details='$sys_upg_details',haz_chemicals_details='$haz_chemicals_details', public_hearing_doc='$public_hearing_doc' WHERE form_id='$form_id'");
...
if(isset($_POST["save2h"])){	
	if((isset($_POST["mfile16"]) && empty($_POST["mfile16"])) || (isset($_POST["mfile17"]) && empty($_POST["mfile17"])) || (isset($_POST["mfile18"]) && empty($_POST["mfile18"])) || (isset($_POST["mfile19"]) && empty($_POST["mfile19"])) || (isset($_POST["mfile16"]) && $_POST["mfile16"]=='2') || (isset($_POST["mfile17"]) && $_POST["mfile17"]=='2') || (isset($_POST["mfile18"]) && $_POST["mfile18"]=='2') || (isset($_POST["mfile19"]) && $_POST["mfile19"]=='2') || (isset($_POST["mfile16"]) && $_POST["mfile16"]=='3') || (isset($_POST["mfile17"]) && $_POST["mfile17"]=='3') || (isset($_POST["mfile18"]) && $_POST["mfile18"]=='3') || (isset($_POST["mfile19"]) && $_POST["mfile19"]=='3')){
		...
		if($sql->num_rows<1){				
			...
			$row=$sql->fetch_array();
			$form_id=$row["form_id"];
			$savequery=$formFunctions->executeQuery($dept,"update pcb_form2_upload set file1='$file1',file2='$file2',file3='$file3',file4='$file4' where form_id='$form_id'");
		}		
		if($savequery){
			...
			if($file1="SC" || $file2="SC" ||  $file3="SC" ||  $file4="SC"){
				$save_query=$formFunctions->executeQuery($dept,"update pcb_form2 set uain='$uain',courier_details='1', sub_date='$today' where form_id='$form_id'") ;
			}else{
				$save_query=$formFunctions->executeQuery($dept,"update pcb_form2 set uain='$uain', sub_date='$today' where form_id='$form_id'");
			}
			if($save_query){
				echo "<script>
					alert('Successfully Saved....');
...
if(isset($_POST["proceed2"])){
	$sql=$formFunctions->executeQuery($dept,"select form_id,save_mode,courier_details from pcb_form2 where user_id='$swr_id' and active='1'");		
	if($sql->num_rows<1){				
		echo "<script>
			alert('Please fill the first part of the form.');
			window.location.href = 'form2.php';
		...
		if($save_mode=="D" && $courier_details==1){
			...
			if($save_query){
				echo "<script>
					alert('Successfully Saved.');
					window.location.href = '../../requires/courier_details.php?dept=pcb&form=2';
				</script>";
			}else{
				echo "<script>
					alert('Something went wrong !!!');
			...
			$save_query=$formFunctions->executeQuery($dept,"update pcb_form2 set sub_date='$today',uain='$uain',save_mode='P' where form_id='$form_id'");
			if($save_query){
				echo "<script>
					alert('Successfully Saved.');
	...
	if($_POST["payment_mode"]==1){
		...
		if(empty($_POST["offline_challan"]) || $_POST["offline_challan"]=='2' || $_POST["offline_challan"]=='3'){
			...
			if($sql->num_rows>0){
				$form_id=$row["form_id"];				
				$offline_challan=$_POST["offline_challan"];$payment_mode=$_POST["payment_mode"];
				$save_query=$formFunctions->executeQuery($dept,"update pcb_form2 set offline_challan='$offline_challan',payment_mode='$payment_mode',save_mode='C',sub_date='$today' where form_id='$form_id'");
				
				if($save_query){
					$uain=$formFunctions->executeQuery($dept,"select uain from pcb_form2 where form_id='$form_id'")->fetch_object()->uain;
