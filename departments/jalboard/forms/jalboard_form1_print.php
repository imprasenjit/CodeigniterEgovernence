...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	if($q->num_rows>0){
		...
		}else if($property_type=="I") $institutional=$property_type_sub_category;
		...
		if(!empty($results["b_add"])){
			...
			$bill_house=$b_add->a;
			$bill_locality=$b_add->b;
			$b_byelane=$b_add->c;
			$bill_ward=$b_add->d;
			$bill_dag=$b_add->e;
			$bill_patta=$b_add->f;
			$bill_mouza=$b_add->g;
			$bill_society=$b_add->h;
			$bill_road=$b_add->i;
			$bill_area=$b_add->j;
			$bill_landmark=$b_add->k;
			$bill_vill=$b_add->l;
			$bill_pincode=$b_add->m;
			$bill_mobile=$b_add->n;			
		}else{				
			$bill_house="";
			$bill_locality="";
	...
	if($pay_mode=="CA") $pay_mode="Cash";
	else if($pay_mode=="CQ") $pay_mode="Cheque";
	else if($pay_mode=="DD") $pay_mode="Demand Draft";
	else $pay_mode="Online payment (Internet Banking / Credit Card / Debit Card)";
...
$printContents=$printContents.'
	...
	<table class="table table-bordered table-responsive">
		...
		<tr>
			<td width="50%">b) Designation : </td>
			<td>'.strtoupper($status_applicant).'</td>
		</tr>
		<tr>
			<td width="50%">c)Father/Spouse Name :  </td>
			<td>'.strtoupper($fat_name).'</td>
		</tr>
		<tr>
			<td width="50%">d) Please Tick the relevant box and furnish supporting document for the same : </td>
			<td>'.strtoupper($documents).'</td>
		</tr>
		<tr>
			<td width="50%" colspan="2">2. Contact Details : </td>
		</tr>
		<tr>
			<td >a) Mobile NO. : </td>
