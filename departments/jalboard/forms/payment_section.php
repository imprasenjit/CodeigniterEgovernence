...
if(isset($_GET["token"])){
	...
	if($check==1){
		...
		echo "<script>
				window.location.href = '".$server_url."departments/jalboard/forms/jalboard_form".$token.".php';
		</script>";	
	}else{
		$submit_payment=$token;
		$reg_fees=$jalboard->query("select reg_fees from jalboard_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->reg_fees;
	}
}else{
	echo "<script>
			alert('Something went wrong!!!');
