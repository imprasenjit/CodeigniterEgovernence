...
if(isset($_POST["save1b"])){		
	$connect_type=$_POST["connect_type"];$pay_mode=$_POST["pay_mode"];	$comm_mode=$_POST["comm_mode"];	$gps_point=$_POST["gps_point"];
	...
	$row=$sql->fetch_array(); 
	if($sql->num_rows<1){			
		$query=$formFunctions->executeQueryInsertID($dept,"insert into ".$table_name."(sub_date,connect_type,is_connection,is_arrear,pay_mode,comm_mode,gps_point,declaration) values ('$today','$connect_type','$is_connection','$is_arrear','$pay_mode','$comm_mode','$gps_point','$declaration')");
		$form_id=$query;
	}else{				
		$form_id=$row["form_id"];	
		$query=$formFunctions->executeQuery($dept,"update ".$table_name." set sub_date='$today', connect_type='$connect_type',is_connection='$is_connection',is_arrear='$is_arrear',pay_mode='$pay_mode', comm_mode='$comm_mode', gps_point='$gps_point', declaration='$declaration' where form_id='$form_id'");
	}
	if($query){
		echo "<script>
			alert('Successfully Saved..');
