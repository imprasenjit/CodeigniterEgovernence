...
$check=$formFunctions->is_already_registered('tcp','6');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=6&dept=tcp';
		</script>";
}else if($check==3){
...
include "save_tcp_form.php";
		...
		$q=$tcp->query("select * from tcp_form6 where user_id='$swr_id' and active='1'") or die($tcp->error);
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from tcp_form_names where form_no='6'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
                         ...
                         <table class="table table-responsive">
								...
								<tr>
									<td colspan="4" class="form-inline">
										To The &nbsp;<input type="text"  class="form-control text-uppercase" name="officer_name" value="<?php echo $officer_name;?>"><br/>
								...
								<tr>
									...
									<button type="submit" class="btn btn-success submit1" name="save6" title="Save it and fill up the form later and Go to the Next Part"  onclick="return confirm('Do you want to save..?')" >Save and Next</button>
									</td>
								</tr>									
							</table>
						</form>
