...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
	...
	$form_id=$_GET["ui"];
	$q=$tcp->query("select * from tcp_form9  where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$tcp->error);
}else if(isset($_GET["uain"])){
	$uain=$_GET["uain"];
	$q=$tcp->query("select * from tcp_form9  where user_id='$swr_id' and uain='$uain'") or die("Error :".$tcp->error);	
}else if(isset($form_id)){
	$form_id=$form_id;
	$q=$tcp->query("select * from tcp_form9 where user_id='$swr_id' and form_id='$form_id'") or die("Error :".$tcp->error);
}else{
	$q=$tcp->query("select * from tcp_form9 where user_id='$swr_id' and active='1' and form_id=form_id") or die("Error :".$tcp->error);
}
	...
	$form_name=$formFunctions->get_formName('tcp','9');
	$assamSarkarLogo=$formFunctions->getAssamSarkarLogo($server_url);
      ...
      <table width="100%" align="center" class="table table-bordered table-responsive" style="margin:0px auto;border-collapse: collapse" border="1">
		<tr>
			...
			We hereby apply for enrolment of our Group or Agency in the name and style as mentioned below, as competent technical personnel to do the various works of schemes Building Permit and supervision in the &nbsp;'.strtoupper($supervision).'<br/><br/>
		...
		<tr>
			<td width="40%">1. Name of persons associated with his/ her personal capacity and rank and personal bio-data:</td><td width="60%">'.$val1.'</td>
		</tr>
		 ';
		if(!empty($results["courier_details"]) && $results["courier_details"] != 1){
			$printContents=$printContents.'
