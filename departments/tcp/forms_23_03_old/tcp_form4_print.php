...
if(!isset($form_id) && !isset($_GET["ui"]) &&  !isset($_GET["token"]) && !isset($_GET["uain"])){
		...
		$form_id=$_GET["ui"];
		$q=$tcp->query("select * from tcp_form4 where user_id='$swr_id' and form_id='$form_id'") or die($tcp->error);
	}else if(isset($_GET["uain"])){
		$uain=$_GET["uain"];
		$q=$tcp->query("select * from tcp_form4 where uain='$uain' and user_id='$swr_id'") or die($tcp->error);
  ...
  $unit_details="Street Name : ".strtoupper($b_street_name1)."  ".strtoupper($b_street_name2)."\nVill/Town : ".strtoupper($b_vill)." , ".strtoupper($b_dist)."\nPin Code : ".$b_pincode."\nMobile Number : +91 ".$b_mobile_no."\nPhone Number : ".$b_landline_std."-".$b_landline_no;
	...
	if($results["payment_mode"]==0){
	 ...
	 $form_name=$cms->query("select form_name from tcp_form_names where form_no='4'")->fetch_object()->form_name; 
