...
if(isset($_GET["token"])){
	...
	if($token==7 || $token==8 || $token==9){
		...
		if($check==1){
			...
			echo "<script>				
						window.location.href = '".$server_url."departments/requires/courier_details.php?form=".$token."&dept=tcp';
				</script>";
		}else if($check==0){
			echo "<script>
		...
		$query="select save_mode from tcp_form". $token ." where form_id='$form_id' and active='1'";
		$check = $formFunctions->executeQuery("tcp",$query);
		$save_mode=$check->fetch_object()->save_mode;
		if($save_mode=="C"){
			echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=tcp';
	...
	if($token>0){
		include "tcp_form".$token."_print.php";
	}
}else{
	echo "<script>
			alert('Something went wrong!!!');
  ...
  <div class="container-fluid avoid_me">
	...
	<div class="navbar-header col-md-12 text-center">	
	  ...
	  <?php if(isset($show_edit)){ ?>			
			<a type="button" href="tcp_form<?php echo $token; ?>.php" class="btn btn-app bg-light-blue"><i class="fa fa-edit"></i>Edit</a>			
			<button type="submit" name="<?php echo "proceed".$token;?>" onclick="return confirm('Do you really want to Submit the form ? Once submitted, You will not be allowed to modify any information provided in the form.')" class="btn btn-app bg-green-active submit1"><i class="fa fa-save"></i>Submit</button>
	  <?php }else{ ?>
