...
$q=$formFunctions->executeQuery($dept,"select * from ".$table_name." where user_id='$swr_id' and active='1' and save_mode='D'");
	if($q->num_rows<1){	
		...
		if($p->num_rows>0){
			...
			$ref_uain=$results["ref_uain"];$ownername=$results["ownername"];$location=$results["location"];$submit_dt=$results["submit_dt"];$receive_dt=$results["receive_dt"];$engineer=$results["engineer"];$engineer_address=$results["engineer_address"];$development_name=$results["development_name"];$development_address=$results["development_address"];$drawings=$results["drawings"];
		}else{
			$form_id="";
			$ref_uain="";$ownername="";$location="";$submit_dt="";$receive_dt="";$officer_name="";$add_line1="";$add_line2="";$engineer="";$engineer_address="";$development_name="";$development_address="";$drawings="";
		}
	}else{			
