...
$save_mode="D";
if($token=="trade"){
	$check=$formFunctions->is_already_registered('water','1');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=1&dept=water';
			</script>";	
	}
	$form_name=$cms->query("select form_name from water_form_names where form_no='1'")->fetch_object()->form_name;
	$sql=$water->query("select form_id,uain,save_mode from water_form1 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$check=$formFunctions->is_already_registered('water','2');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=2&dept=water';
			</script>";	
	}
	$form_name=$cms->query("select form_name from water_form_names where form_no='2'")->fetch_object()->form_name;
	$sql=$water->query("select form_id,uain,save_mode from water_form2 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
	...
	$check=$formFunctions->is_already_registered('water','3');
	if($check==1){
		echo "<script>
					alert('Successfully Submitted');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=3&dept=water';
			</script>";	
	}
	$form_name=$cms->query("select form_name from water_form_names where form_no='3'")->fetch_object()->form_name;
	$sql=$water->query("select form_id,uain,save_mode from water_form3 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uain=$row['uain'];
...
if($save_mode=="C"){
	echo "<script type='text/javascript'>alert('Already Applied !');window.location.href='water_form3_acknow.php';</script>";exit();
}
/*PGI Request to billdesk*/
$returnUrl=$server_url."departments/water/forms/form_payment.php?token=".$token;
$industryName="Department of water";
...
if($sid==18){
$newData="DOIACGOA|".$sentCustomerID."|NA|1.00|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|water001|".$txDateTime."|".$form_name."|NA|NA|".$returnUrl;
}else{
$newData="DOIACGOA|".$sentCustomerID."|NA|100.00|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|water001|".$txDateTime."|".$form_name."|NA|NA|".$returnUrl;
}
...
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=1&dept=water" />
<?php }
	if($formName=="F2"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=2&dept=water" />
<?php } 
	if($formName=="F3"){ ?>
<META http-equiv="refresh" content="5;URL=../requires/acknowledgement.php?form=3&dept=water" />
<?php } ?>  
<?php } ?> 
</head>
