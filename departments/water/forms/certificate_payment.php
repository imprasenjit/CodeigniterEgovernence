...
$is_paid="N";
if($token=="trade"){
	$formName=$cms->query("select form_name from water_form_names where form_no='1'")->fetch_object()->form_name;
	$sql=$water->query("select form_id,uain from water_form1 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uian=$row['uain'];
		#certificate check
		$sqlC=$water->query("select total_fees,is_paid from water_form1_certificates where form_id='$form_id'") or die("Error : ".$water->error);
...
$returnUrl=$server_url."water/certificate_payment.php?token=".$token;
$industryName="Guwahati Jal Board";
$financialYear=calculateFiscalYearForDate(date("Y/m/d"),"7/1","6/30");
$txDateTime=time();
$paymentDetails="";
$txremarks="";
$sentCustomerID=$uian."/C";
$newData="DOIACGOA|".$sentCustomerID."|NA|".$certificate_fees."|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|water001|".$txDateTime."|".$formName."|NA|NA|".$returnUrl;
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from water_form_names where form_no='1'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
