...
if(isset($_GET["token"])){
	$token=$_GET["token"];
	$check=$formFunctions->is_already_registered('fcs',$token);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=fcs';
			</script>";	
	}else if($check==0){
		echo "<script>
		...
		echo "<script>
				window.location.href = '".$server_url."departments/fcs/forms/fcs_form".$token.".php';
		</script>";
	}else{
		$submit_payment=$token;
		$reg_fees=$formFunctions->executeQuery($dept,"select reg_fees from fcs_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->reg_fees;
	}
}else{
	echo "<script>
			alert('Something went wrong!!!');
			window.location.href = '".$server_url."departments/fcs/';
	</script>";
}
?>
<!DOCTYPE html>
											...
											<div class="row">
												<div class="col-md-4 col-md-offset-4 box box-shadow" style="width:30%;padding-bottom:30px">
													<p><b><u>Challan Head</u></b><br/>
														<p>Head of Account :<br/>"0408-Food Storage & Warehousing-<br/>01-Food-000-800-other receipt"</p> <br/><br/>
													</p>
													<div class="row">
