...
if(is_numeric($token)){
	$form=$token;
	$check=$formFunctions->is_already_registered('pcb',$form);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$form."&dept=pcb';
			</script>";	
	}
	$formName=$formFunctions->get_sampleformName('pcb',$form);
	$table_name=$formFunctions->getTableName("pcb",$form);
	
	$reg_fees=100;
	if($token==1 || $token==2 || $token==3 || $token==47 || $token==48 || $token==50){
		
		$results=$pcb->query("select user_id,investment_cost from ".$table_name." where user_id='$swr_id' and active='1'");
	...
	$sql=$pcb->query("select form_id,uain,save_mode from ".$table_name." where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uian=$row['uain'];
...
$returnUrl=$server_url."departments/pcb/forms/form_payment.php?token=".$token;
$industryName="Pollution Control Board";
$financialYear=calculateFiscalYearForDate(date("Y/m/d"),"7/1","6/30");
$txDateTime=time();
$paymentDetails="";
$txremarks="";
$sentCustomerID=$uian."/A";

/* $user_details=$adminFunctions->getUBINdetails($swr_id);
$district=$user_details->b_dist;

$query="select id from offices where jurisdiction LIKE '%$district%' and id!='1' and id!='10'";
$office_results=$formFunctions->executeQuery($dept,$query);
$office_id=$office_results->fetch_object->id;
if($office_id==2){
	$pcb_code="PCB002";
}else if($office_id==3){
	$pcb_code="PCB003";
...
$newData="DOIACGOA|".$sentCustomerID."|NA|".$reg_fees.".00|NA|NA|NA|INR|NA|R|doiacgoa|NA|NA|F|".$industryName."|".$financialYear."|PCB001|".$txDateTime."|".$formName."|NA|NA|".$returnUrl;
