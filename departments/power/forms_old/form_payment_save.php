...
if(!empty($formName)){
	...
	$sqlcte=$pcb->query("select form_id,user_id,save_mode from ".$table_name." where uain='$uian'");
	...
	if($sqlcte->num_rows>0){
		...
		if($save_mode=="P"){
			...
			if($payment_insert){
				$save_query=$pcb->query("update ".$table_name." set save_mode='C',payment_mode='1',sub_date='$today' where form_id='$form_id' and user_id='$swr_id'") or die($pcb->error);
					
				$formFunctions->insert_applications($uian);
				$str=$formFunctions->getEmail_str($uian);
				/*----------------SEND MAIL-----------------*/
