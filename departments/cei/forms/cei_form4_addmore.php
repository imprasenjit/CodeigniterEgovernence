<?php
if(isset($form_id)){
	$part1=$formFunctions->executeQuery($dept,"SELECT id FROM ".$table_name."_enclosure4 WHERE form_id='$form_id'");
	$num = $part1->num_rows;
}else{
	$num=0;
}
...
<script >	/*---------FOR TABLE-1------------*/
	...
	function addMorefunction1(){
		...
		t9.id="txtI"+index;
		t9.name="txtI"+index;
		t9.className = "dob form-control text-uppercase";
...
<?php
if(isset($form_id)){
	$part2=$formFunctions->executeQuery($dept,"SELECT id FROM ".$table_name."_enclosure5 WHERE form_id='$form_id'");
	$num2 = $part2->num_rows;
}else{
	$num2=0;
}
