...
include "save_form26.php";	
include "save_form25.php";	
if(isset($_GET["token"])){
	$token=$_GET["token"];
	$check=$formFunctions->is_already_registered('cei',$token);
	if($check==1){
		echo "<script>
					alert('Already Registered');
					window.location.href = '".$server_url."departments/requires/acknowledgement.php?form=".$token."&dept=cei';
			</script>";	
	}else if($check==0){
		echo "<script>
		...
		echo "<script>
				window.location.href = '".$server_url."departments/cei/forms/cei_form".$token.".php';
		</script>";
	}else{
		$submit_payment=$token;
		if($submit_payment==11){
			$sql1=$formFunctions->executeQuery($dept,"select reg_fees from cei_form11 where user_id='$swr_id' and active='1'");
			
			if($sql1->num_rows>0){
				$reg_fees=$sql1->fetch_object()->reg_fees;
			}else{				
				$reg_fees=100;				
			}
		}
		...
		{
			$reg_fees=$formFunctions->executeQuery($dept,"select reg_fees from cei_form".$token." where user_id='$swr_id' and active='1'")->fetch_object()->reg_fees;
		}
	}
}else{
	echo "<script>
			alert('Something went wrong!!!');
			window.location.href = '".$server_url."departments/cei/';
	</script>";
}
?>
<!DOCTYPE html>
