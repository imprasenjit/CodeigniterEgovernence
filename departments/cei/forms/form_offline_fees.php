...
if(isset($_GET["token"]) && $_GET["token"]=="2"){
	$check=$factory->query("select process_type from factory_form2 a,factory_form2_process b,factory_form2_certificates c where a.user_id='$swr_id' and b.form_id=a.form_id and b.process_type='A' and c.is_paid!='Y'") or die($factory->error);
	if($check->num_rows>0){
		$form_id=$factory->query("select form_id from factory_form2 where user_id='$swr_id' and active='1'")->fetch_object()->form_id;
		$fees=$factory->query("select total_fees from factory_form2_certificates where form_id='$form_id' and is_paid!='Y'")->fetch_object()->total_fees;
		$prev="../index.php";
	}
