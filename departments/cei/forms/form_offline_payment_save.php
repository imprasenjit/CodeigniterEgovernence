...
if(isset($_POST["submit_fees"])){
	...
	if($_POST["payment_mode"]==1){
		echo "<script>
				...
				window.location.href = 'certificate_payment.php?token=2';
			</script>";
		...
		if(empty($_POST["offline_challan"]) || $_POST["offline_challan"]=='2' || $_POST["offline_challan"]=='3'){
			echo "<script>
				...
				window.location.href = 'form_offline_fees.php?token=2';
			</script>";
		}else{
			$offline_fees_challan=clean($_POST["offline_challan"]);
			$save_query=$factory->query("update factory_form2_certificates set offline_fees_challan='$offline_fees_challan',is_paid='Y' where form_id='$form_id'") or die($factory->error);
			if($save_query){					
				...
				echo $factory->error;
				echo "<script>window.location.href = 'form_offline_fees.php?token=2';</script>";
			}
		}								
	}else{
		echo "<script>
