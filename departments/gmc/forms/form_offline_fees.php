...
if(isset($_GET["token"]) && ($_GET["token"]=="trade" || $_GET["token"]>0) ){
	...
	$tableName=$formFunctions->getTableName("gmc",$form);
	
	$check=$gmc->query("select process_type from ".$tableName." a,".$tableName."_process b,".$tableName."_certificates c where a.user_id='$swr_id' and b.form_id=a.form_id and b.process_type='A' and c.is_paid!='Y'") or die($gmc->error);
	if($check->num_rows>0){
		$form_id=$gmc->query("select form_id from ".$tableName." where user_id='$swr_id' and active='1' ")->fetch_object()->form_id;
		$fees=$gmc->query("select total_fees from ".$tableName."_certificates where form_id='$form_id' and is_paid!='Y'")->fetch_object()->total_fees;		
		$prev="../index.php";
	}else{
		echo "<script>
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<h4 class="text-center text-bold" >
									<?php echo $form_name=$cms->query("select form_name from gmc_form_names where form_no='".$form."'")->fetch_object()->form_name; ?>
								</h4>	
							</div>
										...
										<div class="row">
											<div class="col-md-8 col-md-offset-3">
												<div class="form-inline">
													...
													<input type="hidden" name="form_no" value="<?php echo $form;?>" readonly=readonly/>
