...
$is_paid="N";
if($token=="trade"){
	$formName=$cms->query("select form_name from gmc_form_names where form_no='1'")->fetch_object()->form_name;
	$sql=$gmc->query("select form_id,uain from gmc_form1 where user_id='$swr_id' and active='1'");
	$row=$sql->fetch_array();
	if($sql->num_rows>0){
		$form_id=$row['form_id'];
		$uian=$row['uain'];
		#certificate check
		$sqlC=$gmc->query("select total_fees,is_paid from gmc_form1_certificates where form_id='$form_id'") or die("Error : ".$gmc->error);
