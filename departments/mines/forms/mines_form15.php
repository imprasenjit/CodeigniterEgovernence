	...
	$q=$formFunctions->executeQuery($dept,"select * from ".$table_name." where user_id='$swr_id' and active='1'");
		...
		$p=$formFunctions->executeQuery($dept,"select * from ".$table_name." where user_id='$swr_id' and active='0' ORDER BY form_id DESC LIMIT 1");
			if($p->num_rows>0){
				...
				$form_id=$results["form_id"];$indenture_dt=$results["indenture_dt"];$acting_through=$results["acting_through"];$bid_rs=$results["bid_rs"];$words_rupees=$results["words_rupees"];$auction_dt=$results["auction_dt"];$mining_contract=$results["mining_contract"];$words_mining=$results["words_mining"];$cubic_metres=$results["cubic_metres"];
				...
				$village_situated=$results["village_situated"];$sub_division=$results["sub_division"];$land_district=$results["land_district"];$dag_no=$results["dag_no"];$patta_no=$results["patta_no"];$an_area=$results["an_area"];$north=$results["north"];$south=$results["south"];$east=$results["east"];$west=$results["west"];$premises_dt=$results["premises_dt"];$for_term=$results["for_term"];
				...
				$governor_assm=$results["governor_assm"];$surety_sig=$results["surety_sig"]; 
				  ...
				  $auction_dt=""; $mining_contract=""; $words_mining="";$cubic_metres="";$officer_rs=""; $officer_rupees="";$security_name="";
				  $shri=""; $resident="";
				  $re_district="";$veins_seam="";$village_situated="";$sub_division="";
				  $land_district="";$dag_no="";
				  $patta_no="";$an_area="";$north="";
								...
								<form name="myform1" id="myform1" class="submit1" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
									<table id="" class="table table-responsive">
										...
										<tr>
											...
											<td width="25%">Signature of the Permit holder:</td>
											<td><input type="text" name="contra_sig" class=" form-control text-uppercase" value="<?php echo $contra_sig; ?>"></td>
										</tr>
										<tr>
											<td width="25%">For and on behalf of Government of Assam</td>
