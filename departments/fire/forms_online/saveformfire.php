		...
		if($query->num_rows<1){
			...
			if($query2->num_rows>0){ 
              ...
              if($file1=="SC" || $file2=="SC" ||  $file3=="SC" ||  $file4=="SC" ||  $file5=="SC"){
					...
					$save_query=$fire->query("update fire_form1 set courier_details='1',sub_date='$today' where form_id='$form_id'") or die($fire->error);
			...
			$query2=$fire->query("select * from fire_form2 where form_id='$form_id'") or die("Error :". $fire->error);
			  if($query2->num_rows>0){
				  ...
				  if($file1=="SC" || $file2=="SC" ||  $file3=="SC" ||    $file5=="SC"){
					...
					$save=$fire->query("update fire_form2 set courier_details='1',sub_date='$today' where form_id='$form_id'") or die($fire->error);
					
				}else{
				$courier_details=NULL;
				$save_query=$fire->query("update fire_form2 set courier_details='$courier_details' where form_id='$form_id'") or die($fire->error);}
