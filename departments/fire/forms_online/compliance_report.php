			...
			<div class="row">
				...
				<?php 
					if(isset($_GET["token"])){
						...
						$q=$fire->query("select letter_no,letter_date,active from compliance_report where uain='$uain' ORDER BY comp_id DESC LIMIT 1") or die($fire->error);
						if($q->num_rows>0){
							$rows=$q->fetch_object();
							$active=$rows->active;
							if($active=='1'){								
								...
								echo "<script>
									alert('You have already submitted the Compliance Report.');
									window.location.href = '../index.php';
								</script>";	
							}
