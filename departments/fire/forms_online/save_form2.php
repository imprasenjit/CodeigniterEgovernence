...
if(isset($_POST['save3a'])){
   ...
   $surround_prop=json_encode($_POST['surround_prop']);$os_width=json_encode($_POST['os_width']);$site_area=clean($_POST['site_area']);$total_area=clean($_POST['total_area']);$premise_access=clean($_POST['premise_access']);$no_of_floor=clean($_POST['no_of_floor']);$floor_details=clean($_POST['floor_details']);$access_premises=clean($_POST['access_premises']);$width_entry=clean($_POST['width_entry']);$no_of_entrance=clean($_POST['no_of_entrance']);$parking=clean($_POST['parking']);$nearest_station=clean($_POST['nearest_station']);$fire_std=clean($_POST['fire_std']);$fire_land=clean($_POST['fire_land']);$system_details=clean($_POST['system_details']);$water_details=clean($_POST['water_details']);$personnel_details=clean($_POST['personnel_details']);$license_authority=clean($_POST['license_authority']);$other_info=clean($_POST['other_info']);
...
if(isset($_POST['save4a'])){
	...
	if(isset($_POST['save4b'])){
		...
		$w_building=$_POST['w_building'];$emergency=clean($_POST['emergency']);$parking=$_POST['parking'];$nearest_station=clean($_POST['nearest_station']);$tel_no=json_encode($_POST['tel_no']);$details_f_f_system=clean($_POST['details_f_f_system']);$details_w_s=clean($_POST['details_w_s']);$details_p_t=clean($_POST['details_p_t']);$sl_c_details=json_encode($_POST['sl_c_details']);$other_info=clean($_POST['other_info']);
...
if(isset($_POST['save6a'])){ 
		...
		if($sql->num_rows<1){  ////////////table is empty//////////////
			...
			$form_id=$row["form_id"];	
			$query=$fire->query("update fire_form6 set sub_date='$today', owner_name='$owner_name', owner_address='$owner_address' where form_id='$form_id'") OR die("Error: ".$fire->error);	
	}		
	if($query){
	...
	$sql=$fire->query("select form_id from fire_form7 where user_id='$swr_id' and active='1'");
		...
		if($sql->num_rows<1){   
		  ...
		  $update=$fire->query("update fire_form7 set product_clasification='$product_clasification',quantity_stored='$quantity_stored',
			...
			nearest_station='$nearest_station',details_f_f_system='$details_f_f_system',
   ...
   $holding_no=$_POST['holding_no'];$letter_no=clean($_POST['letter_no']);$letter_date=date('Y-m-d',strtotime($_POST['letter_date']));$letter_valid_date=date('Y-m-d',strtotime($_POST['letter_valid_date']));$renewal_year1=clean($_POST['renewal_year1']);$renewal_year2=clean($_POST['renewal_year2']);$nearest_station=clean($_POST['nearest_station']);
	...
	$sql=$fire->query("select form_id from fire_form11 where user_id='$swr_id' and active='1'");
		...
		if($sql->num_rows<1){  ////////////table is empty//////////////
			...
			$query=$fire->query("insert into fire_form11(user_id,sub_date,holding_no,letter_no,letter_date,letter_valid_date,renewal_year1,renewal_year2,nearest_station) values ('$swr_id','$today','$holding_no','$letter_no','$letter_date','$letter_valid_date','$renewal_year1','$renewal_year2','$nearest_station')") OR die("Error : ".$fire->error);		
			
	}else{  ////////////table is not empty//////////////
			$form_id=$row["form_id"];	
			$query=$fire->query("update fire_form11 set sub_date='$today', holding_no='$holding_no', letter_no='$letter_no', letter_date='$letter_date', letter_valid_date='$letter_valid_date', renewal_year1='$renewal_year1', renewal_year2='$renewal_year2', nearest_station='$nearest_station' where form_id='$form_id'") OR die("Error: ".$fire->error);	
	}			
...
if(isset($_POST['save12a'])){
		...
		if($sql->num_rows<1){  ////////////table is empty//////////////
			...
			$query=$fire->query("insert into fire_form12(user_id,sub_date,caller_no,caller_name,occured_date,ocured_time,nearest_station,distance_fire,owner_address,place_occurrence) values ('$swr_id','$today', '$caller_no','$caller_name','$occured_date','$ocured_time','$nearest_station','$distance_fire','$owner_address','$place_occurrence')") OR die("Error : ".$fire->error);		
			
	}else{  ////////////table is not empty//////////////
			$form_id=$row["form_id"];	
			$query=$fire->query("update fire_form12 set caller_name='$caller_name',caller_no='$caller_no',occured_date='$occured_date',ocured_time='$ocured_time',nearest_station='$nearest_station',owner_address='$owner_address',place_occurrence='$place_occurrence',distance_fire='$distance_fire' where form_id='$form_id'") or die("error:".$fire->error); 
	}			
