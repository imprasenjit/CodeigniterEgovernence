...
$check=$formFunctions->is_already_registered('fire','7');
if($check==1){
	echo "<script>
	...
	echo "<script>				
				window.location.href = '".$server_url."departments/requires/courier_details.php?form=7&dept=fire';
		</script>";
}else if($check==3){
	$showtab=10;
}else{
	...
	$row=$q->fetch_assoc();
	$p_o_name="";$ms="";$hsd="";$sk="";$fo="";$file1="";$file2="";$file3="";$file4="";
	if($q->num_rows>0){   
		$form_id=$row['form_id'];$p_o_name=$row['p_o_name'];$p_o_addr=json_decode($row['p_o_addr']);$p_o_addr_s1=$p_o_addr->s1;$p_o_addr_s2=$p_o_addr->s2;$p_o_addr_vt=$p_o_addr->vt;$p_o_addr_blk=$p_o_addr->blk;
			$p_o_addr_dist=$p_o_addr->dist;$p_o_addr_pin=$p_o_addr->pin;
			$q=$fire->query("select * from fire_form7_docs where form_id='$form_id' ") or die($fire->error);
	   $rest=$q->fetch_assoc();
...
<?php require '../../../user_area/includes/css.php';?>
  ...
  <?php require '../../../user_area/includes/aside.php'; ?>
							...
							<div class="panel panel-primary">
								<div class="panel-heading">
									<h4 class="text-center" >
										<strong>FORM NO - VII <br/><?php echo $form_name=$formFunctions->get_formName('fire','7');?> </strong>
									</h4>	
								</div>
								...
								<tr>
									...
									<td class="text-center" colspan="2">
										<button type="submit" style="font-weight:bold" name="save7a" class="btn btn-success submit1">Save and Next</button>
									</td>
									<td></td>
								</tr>
								</table>
	...
	<div id="table2" class="tab-pane <?php echo $tabbtn2; ?>" role="tabpanel">
		...
		<table id="" class="table table-responsive">
	 <tr>
    <td width="25%">5. Classification of petroleum product</td>
  ...
  <tr>
    ...
    <td>        
          <input type="radio" name="type_of_storage" value="Under Ground" id="RadioGroup1_0" <?php if($row['type_of_storage']=='Under Ground' && $row['type_of_storage']!='Above the Ground') echo "checked"; ?>  />
          Under Ground
        <br />       
          <input type="radio" name="type_of_storage" value="Above the Ground" id="RadioGroup1_1" <?php if($row['type_of_storage']=='Above the Ground') echo "checked"; ?> />
          Above the Ground
        </td>
  ...
  <tr>
    ...
    <td><input type="text" class="form-control text-uppercase" placeholder="electrification details" name="electrification_details" id="textfield23" value="<?php echo $row['electrification_details'];?>"  required="required"/>
    </td>
  ...
  <tr>
    ...
    <td><input type="text" class="form-control text-uppercase" name="n_fire_station" placeholder="Name of Fire Station" id="textfield35" value="<?php echo $row['n_fire_station'];?>" /></td>
  </tr>
  <tr> 
    <td>16. Details of the Fire Fighting Equipments available in the premises</td>
    <td>
      <textarea validate="textarea" name="details_f_f_system" class="form-control text-uppercase" id="equipment" cols="45"  required="required" placeholder="Details of Equipments" rows="5"><?php echo $row['details_f_f_system']; ?></textarea></br></td>
    </td>
  ...
  <tr>
    ...
    <td>
      <textarea name="other_info" validate="textarea" class="form-control text-uppercase" id="textfield42" cols="45" rows="5"><?php echo $row['other_info'];?></textarea></br>
    </td>
  </tr>
									...
									<tr>
										...
										<a href="fire_form7.php?tab=1"><button type="submit" class="btn btn-primary">Go Back &amp; Edit</button></a>
											<button type="submit" style="font-weight:bold" name="save7b" class="btn btn-success submit1">Save and Next</button>
										</td>
										<td></td>
									</tr>
								</table>
			...
			<form name="myform1" id="myform1" class="submit1" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
				...
				<tr>
					<td class="text-center" colspan="5">
						<a href="fire_form7.php?tab=2" class="btn btn-primary">Go Back & Edit</a>
						<button type="submit" class="btn btn-success submit1" name="submit7" title="Save it and fill up the form later and Go to the Next Part" onclick="return confirm('Do you want to save..?')">SUBMIT</button>
					</td>
					</tr>
					        </table>
			             </form>
