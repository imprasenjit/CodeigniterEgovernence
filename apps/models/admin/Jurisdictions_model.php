...
class Jurisdictions_model extends CI_Model{
    function add_row($data, $dept_code){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->insert("jurisdiction", $data);
        $dept_db->close();
    }//End of add_row()
     ...
     public function edit_row($id, $data, $dept_code){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->where("id", $id);
        $dept_db->update("jurisdiction", $data);
        $dept_db->close();
        return true;
    }//End of edit_row()
    ...
    function delete_row($id, $dept_code){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->where("id", $id);
        $dept_db->delete("jurisdiction");
        $dept_db->close();
    }//End of if delete_row()
}//End of Jurisdictions_model