...
class Userrequests_model extends CI_Model{
    function add_row($data, $dept_code){
        ...
        $dept_db->insert("forgot_passwords", $data);
        $dept_db->close();
    }//End of add_row()
    
     public function edit_row($id, $data, $dept_code){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->where("request_id", $id);
        $dept_db->update("forgot_passwords", $data);
        $dept_db->close();
        return true;
    }//End of edit_row()
    ...
    function get_rows($dept_code){
        ...
        $dept_db->select("*");
        $dept_db->from("forgot_passwords"); 
        $dept_db->order_by("request_id","DESC");  
        $query = $dept_db->get(); 
        if($query->num_rows() == 0) {
            $dept_db->close();
            return FALSE;
    ...
    function delete_row($id, $dept_code){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->where("request_id", $id);
        $dept_db->delete("forgot_passwords");
        $dept_db->close();
    }//End of if delete_row()
}//End of Userrequests_model