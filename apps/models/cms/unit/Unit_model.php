...
class Unit_model extends CI_Model {
    ...
    function getapprovedunit() {
        ...
        if (!empty($records["result"])) {
            foreach ($records["result"] as $post) {
                ...
                $nestedData["unitaddress"] = $fulladdress->address . " , " . $fulladdress->state . " , " . $fulladdress->dist . " , " . $fulladdress->pin;
    ...
    function getrejectedunit() {
        ...
        if (!empty($records["result"])) {
            foreach ($records["result"] as $post) {
                //print_r($post);die();
                $id = $post->unit_id;
                $action = '<a href="' . base_url("cms/unit/manageunits/$id/") . '" class="btn btn-warning">View</a>&nbsp;&nbsp;<a href="' . base_url("cms/unit/manageunits/$id/") . '" class="btn btn-primary">Edit</a>';
                $address = $post->address;
                $fulladdress = $this->address_model->get($address);
                $nestedData["unit_id"] = $id;
                $nestedData["unitname"] = $post->unit_name;
    ...
    function getmodifiedunit() {
        ...
        $dir = $this->input->post("order")[0]["dir"];
        if (empty($this->input->post("search")["value"])) {
            $records = $this->all_rows($limit, $start, $order, $dir, "modified");
            $totalFiltered = $records["totalrows"];
            $totalData = $totalFiltered;
        } else {
            $search = $this->input->post("search")["value"];
            $records = $this->search_rows($limit, $start, $search, $order, $dir, "modified");
            $totalFiltered = $records["totalrows"];
            $totalData = $totalFiltered;
        }
        $data = array();
    ...
    function all_rows($limit, $start, $col, $dir, $type = NULL) {
        $this->load->database();
        $this->db->select("*");
        $this->db->select("unit.address as unit_address");
        $this->db->limit($limit, $start);
        $this->db->order_by($col, $dir);
        $this->db->from("unit");
        $this->db->join("caf", "unit.caf_id=caf.caf_id", 'left');
        $this->db->join("unit_applicant_details", "unit.unit_id=unit_applicant_details.u_id", 'left');
        if ($type == "approved") {
            $this->db->where("caf.status", "1");
        } else if ($type == "underquery") {
            $this->db->where("unit.query_status", "1");
            ...
            $this->db->where("unit.submitstatus", "1");
            $this->db->where("unit.approvalstatus", "3");
...
//End of all_rows()
    ...
    function create_ubin($unitid) {
        $unitdetails = $this->getunitdetails($unitid);
        $caf_id = $unitdetails->caf_id;
        $caf = $this->getcafpanno($caf_id);
        if (!empty($caf->pan) && strlen($caf->pan) == 10) {
            $pan = $caf->pan;
        } else {
            $pan = "NOPANCARD";
        }
        $mod = $caf_id % 99999;
        $div = floor($caf_id / 99999);
        if ($mod == 0) {
            if ($div > 1) {
                $val = $caf_id - 99999 * ($div - 1);
            } else {
                $val = $caf_id;
            }
        } else {
        ...
        $number = str_pad($unitid, 5, '0', STR_PAD_LEFT);
        $first_sec = $character . $number;

        $ubin = $first_sec . "/" . $pan . "/" . date('m/Y');
        $ubin = strtoupper($ubin);
        return $ubin;
    }

    function dectivate_unit($unit_id) {
        $this->load->database();
        $this->db->where("unit_id", $unit_id);
        $this->db->set("status", "0");
        $this->db->update("unit_master_record");
    }

    function copy_unit_details($unit_id) {
        //First we will deactivate all the units which were there in the master table
        $this->dectivate_unit($unit_id);
        $this->load->database();
        $this->db->from("unit");
        $this->db->join("unit_applicant_details", "unit.unit_id=unit_applicant_details.u_id", 'inner');
        $this->db->join("unit_otherdetails", "unit.unit_id=unit_otherdetails.unit_id", 'inner');
        $this->db->join("unit_landdetails", "unit.unit_id=unit_landdetails.unit_id", 'inner');
        ...
        if($query->num_rows() > 0){
            ...
            $data=array(
                ...
                "app_mobile_no"=>$row->app_mobile_no,
                ...
                "entp_category"=>$row->entp_category,
                "power_requirement"=>$row->power_requirement,
                "entry_time"=>date("Y-m-d H:i:s"),
            );
            $this->db->insert("unit_master_record",$data);
        }
    }

    function approveunit() {        
        if (isset($this->session->edit_unitid)) {
            $unit = $this->getunitdetails($this->session->edit_unitid);
            $ubin = $unit->ubin;
            if (!empty($ubin) && strlen($ubin) > 0) {
                $data = array(
                    "approvalstatus" => "1",
                    "approvetime" => date("Y-m-d H:i:s")
                );
            } else {
                $ubin = $this->create_ubin($this->session->edit_unitid);
                $data = array(
                    "ubin" => $ubin,
                    "approvalstatus" => "1",
                    "approvetime" => date("Y-m-d H:i:s")
            ...
            if ($this->db->affected_rows() > 0) {
                $this->copy_unit_details($this->session->edit_unitid);
                echo json_encode(array("success" => 1, "info" => "Unit Approved successfully!"));
            } else {
    ...
    function rejectunit() {
        if (isset($this->session->edit_unitid)) {
            ...
            $this->db->update('unit', $data);
            if ($this->db->affected_rows() > 0) {
                echo json_encode(array("success" => 1, "info" => "Unit Rejected successfully!"));
            } else {
                echo json_encode(array("success" => 0, "info" => "Something went wrong!"));
            }
        }
    ...
    function resetunit() {
        if (isset($this->session->edit_unitid)) {
            ...
            $this->db->update('unit', $data);
            if ($this->db->affected_rows() > 0) {
                echo json_encode(array("success" => 1, "info" => "Unit Reset successfull! Now the user can edit the application"));
            } else {
                echo json_encode(array("success" => 0, "info" => "Something went wrong!"));
            }
        }
    ...
    function storedetails() {
        ...
        if (empty($unit_name)) {
            ...
            if (empty($unit_house_no)) {
                $error = "Please enter house no / building name. ";
            } else if (empty($unit_street)) {
                $error = "Please enter street name. ";
            } else if (empty($unit_village)) {
                $error = "Please enter village name. ";
            } else if (empty($unit_state)) {
        ...
        if ($error == 1) {
            ...
            if (!isset($addresstype)) {
                $addressdata = array(
                    ...
                    "house_no" => $unit_house_no,
                    "street" => $unit_street,
                    "village" => $unit_village,
                    "state" => $unit_state,
                    "dist" => $unit_dist,
                    "pin" => $unit_pin,
                    "block" => $unit_block,
                ...
                $unitdetails = $this->getunitdetails($this->session->edit_unitid);
                $caf_id = $unitdetails->caf_id;
...
//End of getunitdetails()
    ...
    function storedocuments() {
        ...
        $uploded = moveFile(0, $files);
        if ($uploded) {
            $data = array(
                "documents" => json_encode($uploded),
