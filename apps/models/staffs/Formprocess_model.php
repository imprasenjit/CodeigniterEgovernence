...
class Formprocess_model extends CI_Model{
    ...
    function get_approve_date($dept_code, $form_table, $form_id){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->select("*");
        $dept_db->from($form_table."_process");
        $dept_db->where("form_id", $form_id); 
        $dept_db->where("process_type", "A"); 
        $query = $dept_db->get(); 
        if($query->num_rows() == 0) {
            $dept_db->close();
            return FALSE;
    ...
    function get_issue_row($dept_code, $form_table, $form_id){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->select("*");
        $dept_db->from($form_table."_process");
        $dept_db->where("form_id", $form_id); 
        $dept_db->where("process_type", "I"); 
        $query = $dept_db->get(); 
        if($query->num_rows() == 0) {
            $dept_db->close();
            return FALSE;
    ...
    function get_formrow($dept_code, $form_table, $form_id, $process_date){
        $dept_db = $this->load->database($dept_code, TRUE);
        $dept_db->select("*");
        $dept_db->from($form_table."_process");
        $dept_db->where("form_id", $form_id); 
        $dept_db->order_by("doi","DESC");
        $query = $dept_db->get(); 
        if($query->num_rows() == 0) {
            $dept_db->close();
            return FALSE;
