...
class Deptusers_model extends CI_Model{
    ...
    function get_forward_users($dept,$staff_id,$office_id,$fields){
        $dept_db = $this->load->database($dept, TRUE);
        $dept_db->select($fields);
        $dept_db->from("users"); 
        $dept_db->where("user_id != ", $staff_id ); 
        $dept_db->where("user_id != ", "1" ); 
        $dept_db->where("office_id", $office_id );
        $dept_db->order_by("user_id","ASC");
        $query = $dept_db->get();         
        if($query->num_rows() == 0) {
            $dept_db->close();
            return FALSE;
