...
if($formCertRow) {
	...
	//$lic_exp_year = $formCertRow->$lic_exp_year;
	 ...
	 $alter_other_place = $formCertRow-> alter_other_place;
	 $from_the_year = $formCertRow-> from_the_year;
	 $to_the_year = $formCertRow-> to_the_year;
        $sub_date = $formCertRow->sub_date;
		$alter_name = $formCertRow->alter_name;
		$regn_no= $formCertRow->regn_no;
	   $alter_principal_place = $formCertRow->alter_principal_place;
	...
	else
	{
		...
		$regular_fees=0;
		$lic_exp_year=0;
		$date_of_filling="not found";
		$date_of_o_n_c="not found";
		$sub_date="not found";
		$regn_no="not found";
		$from_the_year="not found";
		$to_the_year="not found";
		$c_business_nature="not found";
		$alter_name="not found";
		$alter_other_place="not found";
		$alter_principal_place="not found";
		$c_remarks="not found";
   //$lic_exp_year = ;
	}	
...
<!--copied from rfs_form2_certifcate.php-->               
		...
		<br/>
			...
			<p align="left">Name of the Firm: <b><?php echo strtoupper($alter_name); ?> (<?php echo strtoupper($c_business_nature); ?>)</b>.</p>
			<p align="left">Date of Establishment: <b><?php echo date('d-m-Y',strtotime($date_of_filling)); ?></b>.</p>
		<!--	<p align="left">Duration of the Firm: <b><?=strtoupper($firm_duration)?></b>.</p>-->
			...
			<thead>
				<tr>
					...
					<td rowspan="2">Despription of documents filled in the statement on Form No. <b>II</b> under section <b>60</b> the I.P. Act, 1932</td>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td>2</td>
					<td><b><?=date('d-m-Y',strtotime($date_of_filling))?></b></td>
				</tr>
			</tbody>
		</table>
		...
		<table width="100%" class="table table-responsive text-center" border="1">
			...
			<tbody>
				<?php 
				    $formRows = $this->forms_model->get_frmrows($this->dept_code, "rfs_form2_members", $form_id);
											...
											<tr>
												...
												<td><?=strtoupper($rows->member_p_address);?> <br/> 
										<td><?=strtoupper($rows->member_doj);?></td>
		...
		<table width="100%" class="table table-responsive text-center" border="1">
			...
			<tbody>
				<tr>
					<td><?php echo strtoupper($alter_principal_place); ?></td>
					<td><?=strtoupper($alter_other_place);?>
					</td>
		...
		<table width="100%" class="table table-responsive text-center" border="1">
			...
			<tbody>
				<tr>
					<td>The Alteration in Constitution or location of the Firm has been recorded w.e.f. <?=date("d-m-Y",strtotime($p_date));?> by <?php echo strtoupper($dist); ?>, Registrar of Firms, Assam,<br/> Guwahati under Sec. 60 and Rule 4(3), of Indian Partnership ct 1932.</td>
					<td>
						<?=strtoupper($c_remarks)?>
					</td>
					<td><center><b>SD/- <?php echo strtoupper($key_person); ?></b><br/>Registrar of Firms, Assam,<br/> Guwahati</center></td>
				</tr>
			</tbody>
		</table>
		<br/>
		...
		<p align="left">** Name of the registarar at the time of Initial Registration of the Firm <b><?=strtoupper($dist)?></b></p>		
	</div>
	<!--copied-->
