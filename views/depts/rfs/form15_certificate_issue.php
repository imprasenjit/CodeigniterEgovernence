...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
	...
	<tr class="text-bold">
		...
		<td>Valid upto</td>
		<td><input type="text" class="dobindia form-control text-uppercase dp" name="valid_upto" id="valid_upto"/></td>
	</tr>
	<tr class="text-bold">
		<td>Validity extended upto</td>
		<td><input type="text" class="dobindia form-control text-uppercase dp" name="validity_extended_upto" id="validity_extended_upto"/></td>
		<td colspan="2"></td>
	</tr>
        <tr>
...
<script type="text/javascript">
	...
	$(document).on("click", "#btn-issuecer", function () {
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/issuecertificates/rfs_form15')?>",
                data: { 
                    uain: uain,
