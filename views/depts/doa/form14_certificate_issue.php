...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
    ...
    <tr class="text-bold">
		<td>Give the Details of seeds</td>
		<td><textarea class="form-control text-uppercase" name="seeds_detail" id="seeds_detail"></textarea></td>
		<td></td>
		<td></td>
	</tr>	
        <tr>
	...
	$(document).on("click", "#btn-issuecer", function () {
            var license_no = $("#license_no").val();
			 var valid_upto = $("#valid_upto").val();
			  var seeds_detail = $("#seeds_detail").val();
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/issuecertificates/doa_form14')?>",
                data: { 
                    uain: uain,
