...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
    ...
    <tbody>
       ...
       <tr class="text-bold">
		...
		<td><input type="text" class="form-control text-uppercase" name="certificate_no" id="certificate_no"/></td>	
		<td>Valid upto</td>
		<td><input type="text" id="valid_upto" name="valid_upto" class="dobindia form-control text-uppercase dp" required="required"></td>
	</tr>
   <tr class="text-bold">
		<td>Renewed upto</td>
		<td><input type="text" id="renewed_upto" name="renewed_upto" class="dobindia form-control text-uppercase dp" required="required"></td>	
		<td></td>
		<td></td>
	</tr>	
        <tr>
...
<script type="text/javascript">
	...
	$(document).on("click", "#btn-issuecer", function () {
			...
			var renewed_upto = $("#renewed_upto").val();
            ...
            $.ajax({
                type: "POST",
             url: "<?=base_url('staffs/issuecertificates/doa_form22')?>",
                    data: { 
                    uain: uain,
