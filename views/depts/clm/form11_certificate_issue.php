...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
 ...
 <tr class="text-bold">
		<td>The concerned Inspector of Legal Metrology</td>
		<td><input type="text" name="concern_ins" id="concern_ins" class="form-control text-uppercase"/></td>
		<td colspan="2"></td>			
	</tr>
        <tr>
	...
	$(document).on("click", "#btn-issuecer", function () {
         //   var file_auth_num = $("#file_auth_num").val();
          //  var reg_no = $("#reg_no").val();
            //var hear_dt = $("#hear_dt").val();
            var concern_ins = $("#concern_ins").val();
			var uain = $("#uain").val();
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/issuecertificates/clm_form11')?>",
                data: { 
                    uain: uain,
                    form_table:form_table,
                    form_id:form_id,
                    //file_auth_num: file_auth_num,
					concern_ins: concern_ins
					//hear_dt: hear_dt
					//reg_no: reg_no
                },
                beforeSend: function () {
