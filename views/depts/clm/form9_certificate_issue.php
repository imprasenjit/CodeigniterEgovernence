...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
  ...
  <tr>
		<td class="text-bold">Registration No.</td>
		<td><input class="form-control text-uppercase" name="reg_no" id="reg_no"/></td>
		<td></td>
		<td></td>
	</tr>
        <tr>
...
<script type="text/javascript">
	...
	$(document).on("click", "#btn-issuecer", function () {
       //     var file_auth_num = $("#file_auth_num").val();
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/Issuecertificates/clm_form7')?>",
                data: { 
                    uain: uain,
