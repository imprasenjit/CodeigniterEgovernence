...
$formRow = $this->forms_model->get_row($this->dept_code, $form_table, $form_id);
	...
	//	$homoeopathic = $formRow->homoeopathic;
	    ...
	    $names_of_drugs = $formRow->names_of_drugs;
...
<html>
   ...
   <!-- copied codes from sdc_form37_certificate_print.php -->
		...
		<h4>[See Rules 73&ndash;B]</h4>
		<h4><b><em>Certificate of Renewal of licence to repack for sale or distribution of drugs being drugs other than those specified in Schedules C and C(1) [excluding those specified in Schedule X]</em></b></h4>
		<br/>
		<table width="100%"  >
			<tr>
		...
		<div align="justify">
			<ol>
				<li>Certified that licence No. <?=strtoupper($prev_lic_no);?> granted on the <?=date('d-m-Y',strtotime($prev_issue_date)); ?> to <?=strtoupper($companyName);?> for the repacking of  the following drugs at the premises situated at <?=strtoupper($address);?> has been renewed from <?=date('d-m-Y',strtotime($issue_date)); ?> to <?=date('d-m-Y',strtotime($valid_upto)); ?>. Names of drugs to be repacked  <?=strtoupper($names_of_drugs);?>.
				</li>
				<li>Names of competent staff : <br/>
				<table>
					<thead>
						<tr>
							<th>Sl No.</th>
