...
<html>
    ...
    <!--copied from sdc_certificate34.php-->
   	<div align="center" style="padding: 10px 30px;width:99%; border:2px solid black;">	
		...
		<h4><b>[FORM 20&ndash;E]</b></h4>
		<h4><b>(See rule 67 EE)</b></h4>
		<h4><b>Certificate of Renewal of Licence to Sell, stock or Exhibit for sale or distribute Homeopathic Medicines</b></h4>
		<br/>
		<table width="100%"  >
			<tr>
		...
		<div align="justify">
			<ol>
				<li>Numbers of Licence and date of issue : <?=strtoupper($license_no);?>, <?=date('d-m-Y',strtotime($issue_date)); ?><br/>
				certified that Licence No. <?=strtoupper($prev_lic_no);?> in Form 20&ndash;C/ 20&ndash;D granted on the <?=date('d-m-Y',strtotime($prev_issue_date)); ?> to <?=strtoupper($companyName);?> for sale of Homeopathic medicines at the premises situated at <?=strtoupper($address);?> has been renewed for a period from <?=date('d-m-Y',strtotime($issue_date)); ?> to <?=date('d-m-Y',strtotime($valid_upto)); ?></li>
				<li>Name of competent person in-charge <?=strtoupper($key_person);?></li>
			</ol>
		</div>
