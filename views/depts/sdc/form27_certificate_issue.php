...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
    ...
    <tbody>
		...
		<tr class="text-bold">
			<td>Regd. No.</td>
			<td><input type="text" class="form-control text-uppercase" name="regd_no" id="regd_no"/></td>
			<td></td>
			<td></td>
	</tr>
...
<script type="text/javascript">
	...
	$(document).on("click", "#btn-issuecer", function () {
			...
			var regd_no = $("#regd_no").val();


            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/issuecertificates/sdc_form27')?>",
                data: { 
                    uain: uain,
