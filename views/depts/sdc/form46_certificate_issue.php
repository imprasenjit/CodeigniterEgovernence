...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
    ...
    <tbody>
       ...
       <tr class="text-bold">
			...
			<td><input type="text" class="dobindia form-control text-uppercase" name="valid_upto" id="license_no"/></td>
			<td></td>
			<td></td>
		</tr>
		<tr class="text-bold">
			<td>Remarks</td>
			<td><input type="text" class="dobindia form-control text-uppercase" name="valid_upto" id="remarks"/></td>
			<td></td>
			<td></td>
		</tr>   
        <tr>
...
<script type="text/javascript">
	...
	$(document).on("click", "#btn-issuecer", function () {
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/issuecertificates/sdc_issue_certificate_common')?>",
                data: { 
                    uain: uain,
