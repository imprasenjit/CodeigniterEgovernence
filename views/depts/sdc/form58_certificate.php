...
$formRow = $this->forms_model->get_row($this->dept_code, $form_table, $form_id);
	...
	if(!empty($formRow->pre)){
		$pre=json_decode($formRow->pre);
		$pre_lic_no=$pre->lic_no;
		$pre_form_num=$pre->form_num;
		$pre_validity=$pre->validity;
		$pre_doi=$pre->doi;
	}else{
		$pre_lic_no="";
		$pre_form_num="";
...
<!--copied from sdc_certificate58.php-->
 <div align="center" style="padding: 10px 30px;width:99%; border:2px solid black;">	
		...
		<br/>
		<p align="justify">M/S <b><?=strtoupper($companyName)?></b></p>
		<p align="justify">Ref : Your Inward Application vide UAIN : <b><?=strtoupper($uain)?></b></p>
		<p align="justify">With Reference to your Inward application, we have to infrom you that your said application is considered &amp; following <b>PRODUCTS PERMISSION</b> have been granted, under the following <b>LICENCES</b>.</p>
		...
		<table width="100%" class="table table-bordered">
			<tr>
				<th width="5%">Sl No.</th>
				<th width="35%">Name of the product</th>
				<th width="40%">Formulation details</th>
...
<?php 
						...
						if($forms_t1_Row){
							...
							<tr >
								...
								<td><?=strtoupper($rows->formula_details);?></td>
								<td><?=strtoupper($rows->pack_size);?></td>
