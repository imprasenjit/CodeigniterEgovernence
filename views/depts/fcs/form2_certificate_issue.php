    ...
    <tbody>
        ...
        <tr>
			...
			<tr class="text-bold">
				...
				<td><input type="text" name="license_no" id="license_no" required="required" class="form-control"/></td>
				<td></td>
				<td class="form-inline"></td>			
		</tr>	
		<tr class="text-bold">
				<td>Validity date from</td>
				<td><input type="text" name="validity_date_from" id="validity_date_from" required="required" class="dob form-control text-uppercase"/></td>
				<td></td>
				<td class="form-inline"></td>			
		</tr>			
		<tr class="text-bold">
				<td>Validity date to</td>
				<td><input type="text" name="validity_date_to" id="validity_date_to" required="required" class="dob form-control text-uppercase"/></td>
				<td></td>
				<td class="form-inline"></td>			
		</tr>
       <tr>
...
<script type="text/javascript">
	...
	$(document).on("click", "#btn-issuecer", function () {
			...
			var license_no = $("#license_no").val();
            var validity_date_from = $("#validity_date_from").val();
            var validity_date_to = $("#validity_date_to").val();
            var uain = $("#uain").val();
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/issuecertificates/fcs_form2')?>",
                data: { 
                    uain: uain,
