...
<table id="table-5" style="display:none" class="table table-bordered table-responsive">
    ...
    <tbody>
      ...
      <tr class="text-bold">
		<td>Plan number</td>
		<td><input type="text" name="reg_no" id="plan_no" class="form-control text-uppercase" /></td>
		<td></td>
		<td></td>			
	</tr>
	<tr class="text-bold">
		<td>Consist of</td>
		<td><input type="text" name="reg_no" id="consist_of" class="form-control text-uppercase" /></td>
		<td></td>
		<td></td>			
	</tr>
        <tr>
...
<script type="text/javascript">
	...
	$(document).on("click", "#btn-issuecer", function () {
            var plan_no = $("#reg_no").val();
            var consist_of = $("#reg_no").val();
            ...
            $.ajax({
                type: "POST",
                url: "<?=base_url('staffs/issuecertificates/labour_form5')?>",
                data: { 
                    uain: uain,
